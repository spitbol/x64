...
static int
sioarg(int ioflg, struct ioblk *ioptr, struct scblk *scptr)
{
    int lastdash = 0;
    word cnt, v, share;
    char ch, *cp;
    ...
    while(cnt < scptr->len) {
        ...
        case 'B':
            ...
            if(v > 0 && ((v + sizeof(word) - 1) & ~(sizeof(word) - 1)) <=
                            (maxsize - BFSIZE))
                ioptr->pid = v;
            else
                return -1;
...
word
scnint(char *str, word len, word *intptr)
...
zysfc()
{
    ...
    struct scblk *scb1 = XL(struct scblk *);
    struct scblk *scb2 = XR(struct scblk *);
...
again:
    ...
    if(length_fname || fd_spec) {
        ...
        save_iob = 0;
        bfblksize =
            (BFSIZE + tioblk.pid + sizeof(word) - 1) & ~(sizeof(word) - 1);
