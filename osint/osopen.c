...
osopen(struct ioblk *ioptr)
{
     ...
     */
        ...
        savech
            = make_c_str(&cp[len]); /*   else temporarily terminate filename */
        if(ioptr->flg1 & IO_OUP) {  /*  output file */
            ...
            if(len == 1 && *cp == '-') {
                 ...
                 *   open for read/write so we can fill buffer
                 * after a seek (even if this is a write only from
                 * file from the user's point of view) if file is
                 * not buffered and not appending or updating, use
                 * O_WRONLY instead of O_RDWR.
                 */
                int mode = O_CREAT; /* create file if it doesn't
                                       exist */
                ...
                fd = spit_open(cp, mode, ioptr->share /* 0666 */,
                               ioptr->action);
            }
            ...
            if(len == 1 && *cp == '-') {
                ...
                fd = spit_open(cp, O_RDONLY, ioptr->share /* 0 */,
                               ioptr->action);
        }
    ...
    /*
       ...
       /    o  if output file is a TTY device, set the IO_WRC flag (no
       buffering) /    o  if IO_WRC flag set, throw away the buffer /    o  if
       output, append and not pipe, seek to end of file.
       /
