...
word
osread(word mode, word recsiz, struct ioblk *ioptr, struct scblk *scptr)
        ...
        if(mode == 1) { /* line mode */
            ...
            if(ioptr->flg2 & IO_PIP) { /* if pipe, read 1 char at a time */
                ...
                if(n > 0) {
                    ...
                    for(i = cnt; i--;) {   /* scan for EOL */
                        if(*cp++ == EOL) { /* if end of line (or EOF) */
                            findeol = 0;
                            cnt -= i + 1;
                            if(testty(fdn))
                                doset(ioptr, cp - scptr->str - n,
                                      1); /* backup file to point following
                                             line */
                            break;
                        }
                    }
                    ...
                    /*
                     * if we didn't see an eol, and this operating system does
                     * not read a line at time (through eol) automatically,
                     * then we have to keep reading one character at time until
                     * we find the eol.
                     */
