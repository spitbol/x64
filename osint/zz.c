...
void
prtdif(char *name, long old, long new, long listed)
{
...
void
prtdifr(char *name, double old, double new, long listed)
{
    /* print old and new values of named register */
    fprintf(stderr, "%s:", name);
    prtreal(old);
...
*/
   ...
   backward for last statement that changed a register. */
        prtnl();
        if(save_xl != last_xl) {
            prtdif("XL.esi", last_xl, save_xl, listed);
            listed += 1;
        }
        if(save_xr != last_xr) {
            prtdif("XR.edi", last_xr, save_xr, listed);
            listed += 1;
        }
        ...
        if(save_w0 != last_w0) {
            prtdif("W0.eax", last_w0, save_w0, listed);
            listed += 1;
        }
        if(save_wa != last_wa) {
            prtdif("WA.ecx", last_wa, save_wa, listed);
            listed += 1;
        }
        if(save_wb != last_wb) {
            prtdif("WB.ebx", last_wb, save_wb, listed);
            listed += 1;
        }
        if(save_wc != last_wc) {
            prtdif("WC.edx", last_wc, save_wc, listed);
            listed += 1;
        }
        if(save_ra != last_ra) {
            prtdifr("RA    ", last_ra, save_ra, listed);
            listed += 1;
        }
        prtnl();
    }
