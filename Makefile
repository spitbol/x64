...
# run preprocessor to get gas for gas as target
...
# run gas definer to resolve system dependencies in sys
	$(BASEBOL) -u $(TARGET)_gas  def.spt <sys.pre >sys.def
# run gas definer to resolve system dependencies in sbl
	$(BASEBOL) -u $(TARGET)_gas  def.spt <sbl.s >sbl.def
# combine sys.s,min.s, and err.s to get sincle assembler source file
	cat <sys.def >spitbol.s
	cat <sbl.def >>spitbol.s
	cat <err.s >>spitbol.s
