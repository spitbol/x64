...
*/
static int
find_match (hash_prefix, hash_character)
     int hash_prefix;
	...
	{			/* table, try to add it.    */
	  ...
	  if (next_code > max_code)	/* output the last string       */
	    {			/* after adding the new one */
	      ...
	      memset ((void *) code_value, -1,
		      table_size * sizeof (short int));
	      next_code = 256;
	    }
	}
    }
...

{
  ...
  switch (whence)
    ...
    case 1:			/* relative to current position */
      target = offset +
	(bfptr ? bfptr->offset +
	 bfptr->next : LSEEK (ioptr->fdn, (FILEPOS) 0, 1));
      break;
...

{
  ...
  if (lmodstk)
    {
      if (brk ((char *) topmem) < 0)
	{			/* restore topmem to its prior state        */
   ...
   */
      ...
      if (ioptr->flg1 & IO_OUP)	/*  output file             */
	{
	  ...
	  if (len == 1 && *cp == '-')
		...
		mode |= O_TRUNC;	/* truncate existing file */
	      ...
	      fd =
		spit_open (cp, mode, ioptr->share /* 0666 */ , ioptr->action);
	    }
      ...
      else			/* input-only file              */
	{
	  ...
	  if (len == 1 && *cp == '-')
	    ...
	    fd =
	      spit_open (cp, O_RDONLY, ioptr->share /* 0 */ , ioptr->action);
	}
...
#if PIPES
	  if (ioptr->flg2 & IO_PIP)
	    {			/* if pipe, read 1 char at a time */

	      do
		{		/* because there's no chance to backup */
		  n = read (fdn, &c, 1);
		  if (n > 0)
		    {
		      if (ioptr->flg2 & IO_LF)
			  ...
			  if (eol2)
			    {
			      if (i && *cp == eol2)
				cp++;
			      else if (testty (fdn) && read (fdn, &c, 1) == 1
				       && c != eol2)
...
extern FILEPOS doset
(struct ioblk * ioptr, FILEPOS offset, int whence);
...
extern int expand
(File_handle fd, unsigned char * startadr, uword size);
...
extern word osread
(word mode, word recsiz, struct ioblk * ioptr, struct scblk * scptr);
extern void oswait (int pid);
extern word oswrite
(word mode, word linesiz, word recsiz, struct ioblk * ioptr,
	 struct scblk * scptr);
...
extern long savestart
(File_handle fd, char *bufp, unsigned int size);
...
extern int sioarg
(int ioflg, struct ioblk * ioptr, struct scblk * scptr);
extern File_handle spit_open
(char *Name, Open_method Method, File_mode Mode, int Access);
extern void startbrk (void);
extern void startup (char *code, char *stack);
...
zysif ()
{
  ...
  if (fnscb)
	...
	return EXIT_1;
      ...
      *savecp = '\0';
      file = fnscb->str;
      fd = spit_open (file, O_RDONLY, IO_PRIVATE | IO_DENY_WRITE, IO_OPEN_IF_EXISTS);	/* Open file */
      if (fd < 0)
	{
	  ...
	  while (trypath (fnscb->str, file))
	    {
	      fd =
		spit_open (file, O_RDONLY, IO_PRIVATE | IO_DENY_WRITE,
			   IO_OPEN_IF_EXISTS);
	      if (fd >= 0)
		break;
	    }
	  ...
	  if (i)
	    {
	      ...
	      fd =
		spit_open (filebuf, O_RDONLY, IO_PRIVATE | IO_DENY_WRITE,
			   IO_OPEN_IF_EXISTS);
	    }
	}
      if (fd < 0 && sfn && sfn[0])
...
zysio ()
{
  ...
  if (tioblk.typ)
      ...
      if (save_iob == 0)
	{
	  ...
	  iob->fnm = MP_OFF ((tioblk.flg2 & IO_ENV) ? XL (struct scblk *) :	/* filearg 1       */
			     XR (struct scblk *), struct scblk *);	/* filename  */

	  iob->pid = 0;		/* process id           */
	  iob->bfb = MP_OFF (bfb, struct bfblk *);	/* buffer               */ iob->fdn = tioblk.fdn;	/* file descriptor      */
  ...
  switch (type)
    ...
    case BL_XN:		/* XNBLK    external block                                              */
      result->xnb.xnlen = ((result->xnb.xnlen + sizeof (mword) - 1) &
			   -sizeof (mword)) + FIELDOFFSET (struct xnblk,
							   xnu.xndta[0]);
 ...
 */
mword
loadDll (dllName, fcnName, ppfnProcAddress)
     char *dllName;
     char *fcnName;
     PFN *ppfnProcAddress;
{
...
File_handle
spit_open (Name, Method, Mode, Action)
     char *Name;
     Open_method Method;
     File_mode Mode;
     int Action;
 ...
 */
word
saveend (word * stkbase, word stklen)
{
  word result;
 ...
 */
long
savestart (int fd, char *bufp, unsigned int size)
{
  struct exec *ep;
  long fpos, position;
  long imagesize;
     ...
     char **pp;
{
  ...
  *pp = (char *) 0;		/* no result sbrk value */
  ...
  while (n >= sizeof (word))
    {				/* Word is minimum allocation unit */
     ...
     int fd;
{
  ...
  if (testty (fd) && (pos = LSEEK (fd, (FILEPOS) 0, 1)) >= 0)
	...
	reload_verserr:
	  ...
	  write (STDERRFD,
		 ((svfheader.version >> VWBSHFT) & 0xF) == 2 ? "32" : "64",
		 2);
