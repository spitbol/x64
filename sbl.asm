...
/*
	...
	this version has the same functionality as the previous release, but with many internal code changes.
	support for x86-64 has been added, but is not currently working.  the description of the minimal language
	formerly found here as comments is now to be found in the file minimal-reference-manual.html
	...
	the source is now maintained using git, so going forward the detailed revision history will be
	recorded in the git commit logs, not in this file.
...
/*
	...
	this program is a translator for a version of the snobol4 programming language. language details are contained in
	the manual macro spitbol by dewar and mccann, technical report 90, university of leeds 1976.
	the implementation is discussed in dewar and mccann, macro spitbol - a snobol4 compiler, software practice and
	experience, 7, 95-113, 1977.  the language is as implemented by the btl translator
	(griswold, poage and polonsky, prentice hall, 1971) with the following principal exceptions.
	...
	5)   the keyword fullscan is not provided and all pattern matching takes place in fullscan mode (i.e. with no
	    heuristics applied).

	6)   a series of expressions separated by commas may be grouped within parentheses to provide a selection
	    capability. the semantics are that the selection assumes the value of the first expression within it
	    which succeeds as they are evaluated from the left.  if no expression succeeds the entire statement fails
	...
	the method used in this program is to translate the source code into an internal pseudo-code (see following
	section). an interpretor is then used to execute this generated pseudo-code. the nature of the snobol4 language
	is such that the latter task is much more complex than the actual translation phase. accordingly, nearly all the
	code in the program section is concerned with the actual execution of the snobol4 program.
*/
	ejc
/*
	...
	the interpretive pseudo-code consists of a series of address pointers. the exact format of the code is
	described in connection with the cdblk format. the purpose of this section is to give general insight into
	the interpretive approach involved.

	the basic form of the code is related to reverse polish.  in other words, the operands precede the operators which
	are zero address operators. there are some exceptions to these rules, notably the unary not operator and the
	selection construction which clearly require advance knowledge of the operator involved.

	the operands are moved to the top of the main stack and the operators are applied to the top stack entries. like
	other versions of spitbol, this processor depends on knowing whether operands are required by name or by value
	and moves the appropriate object to the stack. thus no name/value checks are included in the operator circuits.

	the actual pointers in the code point to a block whose first word is the address of the interpretor routine
	to be executed for the code word.

	in the case of operators, the pointer is to a word which contains the address of the operator to be executed. in
	the case of operands such as constants, the pointer is to the operand itself. accordingly, all operands contain
	a field which points to the routine to load the value of the operand onto the stack. in the case of a variable,
	there are three such pointers. one to load the value, one to store the value and a third to jump to the label.

	the handling of failure returns deserves special comment.  the location flptr contains the pointer to the location
	on the main stack which contains the failure return which is in the form of a byte offset in the current
	code block (cdblk or exblk). when a failure occurs, the stack is popped as indicated by the setting of flptr and
	control is passed to the appropriate location in the current code block with the stack pointer pointing to the
...
/*
	...
	in general, a variable is a location containing such a pointer (although in the case of trace associations this
	is modified, see description of trblk).

	the following is a list of possible datatypes showing the type of block used to hold the value. the details of
...
/*
	...
	during the course of evaluating expressions, it is necessary to generate names of variables (for example
	on the left side of a binary equals operator). these are not to be confused with objects of datatype name which
	are in fact values.

	from a logical point of view, such names could be simply represented by a pointer to the appropriate value cell.
	however in the case of arrays and program defined datatypes, this would violate the rule that there must be
	no pointers into the middle of a block in dynamic store.  accordingly, a name is always represented by a base and
	offset. the base points to the start of the block containing the variable value and the offset is the
	offset within this block in bytes. thus the address of the actual variable is determined by adding the base
	and offset values.

	the following are the instances of variables represented
	in this manner.

	1)   natural variable base is ptr to vrblk offset is *vrval

	2)   table element    base is ptr to teblk offset is *teval

	3)   array element    base is ptr to arblk offset is offset to element

	4)   vector element   base is ptr to vcblk offset is offset to element

	5)   prog def dtp     base is ptr to pdblk offset is offset to field value

	in addition there are two cases of objects which are like variables but cannot be handled in this manner.
	these are called pseudo-variables and are represented with a special base pointer as follows=

	expression variable   ptr to evblk (see evblk)

	keyword variable	     ptr to kvblk (see kvblk)

	pseudo-variables are handled as special cases by the access procedure (acess) and the assignment procedure
...
/*
	...
	the static area builds up from the bottom and contains data areas which are allocated dynamically but are never
	deleted or moved around. the macro-program itself uses the static area for the following.
	...
	in addition, the system procedures may use this area for input/output buffers, external functions etc. space in
	the static region is allocated by calling procedure alost

	the following global variables define the current location and size of the static area.

	statb		     address of start of static area
	state		     address+1 of last word in area.

	the minimum size of static is given approximately by 12 + *e_hnb + *e_sts + space for alphabet string
...
/*
	...
	the dynamic area is built upwards in memory after the static region. data in this area must all be in standard
	block formats so that it can be processed by the garbage collector (procedure gbcol). gbcol compacts blocks down
	in this region as required by space exhaustion and can also move all blocks up to allow for expansion of the
	static region.  with the exception of tables and arrays, no spitbol
	object once built in dynamic memory is ever subsequently modified. observing this rule necessitates a copying
	action during string and pattern concatenation.

	garbage collection is fundamental to the allocation of space for values. spitbol uses a very efficient garbage
	collector which insists that pointers into dynamic store should be identifiable without use of bit tables,
	marker bits etc. to satisfy this requirement, dynamic memory must not start at too low an address and lengths
	of arrays, tables, strings, code and expression blocks may not exceed the numerical value of the lowest dynamic
	address.

	to avoid either penalizing users with modest requirements or restricting those with greater needs on
	host systems where dynamic memory is allocated in low addresses, the minimum dynamic address may be specified
	sufficiently high to permit arbitrarily large spitbol objects to be created (with the possibility in extreme
	cases of wasting large amounts of memory below the start address). this minimum value is made available
	in variable mxlen by a system routine, sysmx.  alternatively sysmx may indicate that a
	default may be used in which dynamic is placed at the lowest possible address following static.
	...
	dname	last available location + 1 dnamb is always higher than state
		since the alost procedure maintains some expansion space above state.

	*** dnamb must never be permitted to have a value less than that in mxlen ***

	space in the dynamic region is allocated by the alloc procedure. the dynamic region may be used by system
	procedures provided that all the rules are obeyed.  some of the rules are subtle so it is preferable for
	osint to manage its own memory needs. spitbol procs obey rules to ensure that no action can cause a garbage
	collection except at such times as contents of xl, xr and the stack are +clean+ (see comment before utility
	procedures and in gbcol for more detail). note that calls of alost may cause garbage collection (shift
	of memory to free space). spitbol procs which call system routines assume that they cannot precipitate
...
/*
	...
	(cp)		code pointer register. used to hold a pointer to the current
			location in the interpretive pseudo code (i.e. ptr into a cdblk).

	(xl,xr)		general index registers. usually used to hold pointers to blocks in dynamic storage. an important
			restriction is that the value in xl must be collectable for a garbage collect call.
			a value is collectable if it either points outside the dynamic area, or if it points to
			the start of a block in the dynamic area.

	(xs)		stack pointer. used to point to the stack front. the stack may build up or down
			and is used to stack subroutine return points and other recursively saved data.
	...
	(ia)		used for all signed integer arithmetic, both that used by the translator and that arising from
...
/*
	...
	in the spitbol translator, the following conditional assembly symbols are referred to. to incorporate the
	features referred to, the minimal source should be prefaced by suitable conditional assembly symbol
	definitions. in all cases it is permissible to default the definitions in which case the additional features
	will be omitted from the target code.
*/
...
/*
	...
	this section starts with descriptions of the operating system dependent procedures which are used by the spitbol
	translator. all such procedures have five letter names beginning with sys. they are listed in alphabetical order.

	all procedures have a  specification consisting of a model call, preceded by a possibly empty list of register
	contents giving parameters available to the procedure and followed by a possibly empty list of register contents
	required on return from the call or which may have had their contents destroyed. only those registers explicitly
	mentioned in the list after the call may have their values changed.

	the segment of code providing the external procedures is conveniently referred to as osint (operating system
	interface). the sysxx procedures it contains provide facilities not usually available as primitives in
	assembly languages. for particular target machines, implementors may choose for some minimal opcodes which
	 do not have reasonably direct translations, to use calls of additional procedures which they provide in osint.
	e.g. mwb or trc might be translated as jsr sysmb, jsr systc in some implementations.

	in the descriptions, reference is made to --blk formats (-- = a pair of letters). see the spitbol
	definitions section for detailed descriptions of all such block formats except fcblk for which sysfc should
	be consulted.

	section 0 contains inp,inr specifications of internal procedures,routines. this gives a single pass translator
	information making it easy to generate alternative calls in the translation of jsr-s for procedures of different
...
/*
	...
	if the conditional assembly symbol .csax is defined, this routine is called immediately after execution and
	before printing of execution statistics or dump output.  purpose of call is for implementor to determine and
	if the call is not required it will be omitted if .csax is undefined. in this case sysax need not be coded.
...
/*
	sysbs is used to implement the snobol4 function backspace if the conditional assembly symbol .cbsp is defined.
	the meaning is system dependent. in general, backspace repositions the file one record closer to the beginning
	of file, such that a subsequent read or write will operate on the previous record.
	...
	the second error return is used for files for which backspace is not permitted. for example, it may be expected
...
/*
	...
	called after initial spitbol compilation and before commencing execution in case osint needs
	to assign files or perform other necessary services.  osint may also choose to send a message to online
...
/*
	...
	sysci is an optional osint routine that causes spitbol to call sysci to convert integer values to strings, rather
	than using the internal spitbol conversion code.

	this code may be less efficient on machines with hardware conversion instructions and in such cases, it may be an
	advantage to include sysci.  the symbol .cnci must be defined if this routine is to be used.

	the rules for converting integers to strings are that positive values are represented without any sign, and
	there are never any leading blanks or zeros, except in the case of zero itself which is represented as a single
	zero digit.  negative numbers are represented with a preceeding minus sign.  there are never any trailing
...
/*
	...
	syscr is an optional osint routine that causes spitbol to call syscr to convert real values to strings, rather
	than using the internal spitbol conversion code. this code may be desired on machines where the integer size
	is too small to allow production of a sufficient number of significant digits.  the symbol .cncr must be defined
	if this routine is to be used.

	the rules for converting reals to strings are that positive values are represented without any sign, and
	there are never any leading blanks or zeros, except in the case of zero itself which is represented as a single
	zero digit.  negative numbers are represented with a preceeding minus sign.  there are never any trailing
	blanks, or trailing zeros in the fractional part.  conversion cannot fail.
...
/*
	...
	sysdm is called by a spitbol program call of dump(n) with n ge 4.	its purpose is to provide a core dump.
	n could hold an encoding of the start adrs for dump and amount to be dumped e.g.	 n = 256*a + s , s = start adrs
...
/*
	...
	sysdt is used to obtain the current date. the date is returned as a character string in any format appropriate
	to the operating system in use. it may also contain the current time of day. sysdt is used to implement the
	...
	the format of the block is like an scblk except that the first word need not be set. the result is copied
...
/*
	...
	sysea may not return if interface chooses to retain control. closing files via the fcb chain will be the
...
/*
	...
	sysef is used to write a page eject to a named file. it may only be used for files where this concept makes
	sense. note that sysef is not normally used for the standard output file (see sysep).
...
/*
	...
	sysej is called once at the end of execution to terminate the run. the significance of the abend and
	code values is system dependent. in general, the code value should be made available for testing, and the
	abend value should cause some post-mortem action such as a dump. note that sysej does not return to its caller.
...
/*
	...
	sysem is used to obtain the text of err, erb calls in the source program given the error code number. it is allowed
	...
	the returned value is a pointer to a block in scblk format except that the first word need not be set. the
	string is copied into dynamic memory on return.  if the null string is returned either because sysem does
	not provide error message texts or because wa is out of range, spitbol will print the string stored in errtext
	keyword.
*/
	ejc
...
/*
	...
	sysen is used to implement the snobol4 function endfile.  the meaning is system dependent. in general, endfile
	implies that no further i/o operations will be performed, but does not guarantee this to be the case. the file
	should be closed after the call, a subsequent read or write may reopen the file at the start or it may be
	...
	the second error return is used for files for which endfile is not permitted. for example, it may be expected
	that the standard input and output files are in this category.
*/
	ejc
...
/*
	...
	the arguments are stored on the stack with the last argument at 0(xs). on return, xs
	is popped past the arguments.

	the form of the arguments as passed is that used in the spitbol translator (see definitions and data structures
	section). the control block format is also described (under efblk) in this section.

	there are two ways of returning a result.

	1)   return a pointer to a block in dynamic storage. this block must be in exactly correct format, including
	    the first word. only functions written with intimate knowledge of the system will return in this way.

	2)   string, integer and real results may be returned by pointing to a pseudo-block outside dynamic memory.
	    this block is in icblk, rcblk or scblk format except that the first word will be overwritten
	    by a type word on return and so need not be correctly set. such a result is
	    copied into main storage before proceeding.  unconverted results may similarly be returned in a
	    pseudo-block which is in correct format including type word recognisable by garbage collector since
...
/*
	...
	see also sysio input and output have 3 arguments referred to as shown
	...
	the scheme described in the spitbol user manual should be adopted when possible. the preferred form is
	...
	_i_ is some form of channel identification used in the
	  absence of _f_ to associate the variable with a file allocated dynamically by jcl commands at
	  spitbol load time.
	...
	another delimiter (see iodel	equ  * early in definitions section).

	sysfc is called when a variable is input or output associated to check file arg1 and file arg2 and
	to  report whether an fcblk (file control block) is necessary and if so what size it should be.
	this makes it possible for spitbol rather than osint to allocate such a block in dynamic memory if required
	 or alternatively in static memory.

	the significance of an fcblk , if one is requested, is entirely up to the system interface. the only restriction
	is that if the fcblk should appear to lie in dynamic memory, pointers to it should be proper pointers to
	the start of a recognisable and garbage collectable block (this condition will be met if sysfc requests
	spitbol to provide an fcblk).  an option is provided for osint to return a pointer in
	xl to an fcblk which it privately allocated. this ptr will be made available when i/o occurs later.
	private fcblks may have arbitrary contents and spitbol stores nothing in them.
*/
	ejc
/*
	the requested size for an fcblk in dynamic memory should allow a 2 word overhead for block type and
	length fields. information subsequently stored in the remaining words may be arbitrary if an xnblk (external
	non-relocatable block) is requested. if the request is for an xrblk (external relocatable block) the
	contents of words should be collectable (i.e. any apparent pointers into dynamic should be genuine block
	pointers). these restrictions do not apply if an fcblk is allocated outside dynamic or is not allocated at all.
	if an fcblk is requested, its fields will be initialised to zero before entry to sysio with the exception of
	words 0 and 1 in which the block type and length fields are placed for fcblks in dynamic memory only.
	...
	if file arg1 is null (standard input/output file), sysfc is called to check non-null file arg2 but any request
	for an fcblk will be ignored, since spitbol handles the standard files specially and cannot readily keep fcblk
	pointers for them.

	filearg1 is type checked by spitbol so further checking may be unneccessary in many implementations.

	file arg2 is passed so that sysfc may analyse and check it. however to assist in this, spitbol also passes
	 on the stack the components of this argument with file name, _f_ (otherwise null) extracted and stacked
	first.

	the other fields, if any, are extracted as substrings, pointers to them are stacked and a count of all items
	stacked is placed in wc. if an fcblk was earlier allocated and pointed to via file arg1, sysfc is also
...
/*
	...
	possible usages-
	1. provide visible screen icon of garbage collection in progress
	2. inform virtual memory manager to ignore page access patterns during garbage collection.  such accesses
	   typically destroy the page working set accumulated by the program.
	3. inform virtual memory manager that contents of memory freed by garbage collection can be discarded.
...
/*
	...
	that where all arguments are null in which case syshs returns an scblk containing name of computer,
	name of operating system and name of site separated by colons. the scblk need not have a correct first field
	as this is supplied on copying string to dynamic memory.  spitbol does no argument checking but does provide a
	single error return for arguments checked as erroneous by osint. it also provides a single execution error
	return. if these are inadequate, use may be made of the minimal error section direct as described in minimal
	...
	result to be stacked which is pointed at by xr, and a return causing spitbol statement failure. if a returned
	result is in dynamic memory it must obey garbage collector rules. the only results copied on return
...
/*
	...
	this routine should return strings to head the standard printer output. the first string will be appended to
	a heading line of the form

	    macro spitbol version v.v

	supplied by spitbol itself. v.v are digits giving the major version number and generally at least a minor
	version number relating to osint should be supplied to give say
	...
	the date and time of the run.  optionally the strings may include site name of the
	the implementor and/or machine on which run takes place, unique site or copy number and other information as
	appropriate without making it so long as to be a nuisance to users.

	the first words of the scblks pointed at need not be correctly set.
...
/*
	...
	sysif is used for include file processing, both to inform the interface when a new include file is desired, and
	when the end of file of an include file has been reached and it is desired to return to reading from the previous
	nested file.

	it is the responsibility of sysif to remember the file access path to the present input file before switching to
	...
	register xl points to an scblk containing the name of the include file to which the interface should switch.  data
	is fetched from the file upon the next call to sysrd.  sysif may have the ability to search multiple libraries
	for the include file named in (xl).  it is therefore required that the full path name of the file where the
	file was finally located be returned in (xr).  it is this name that is recorded along with the source statements,
...
/*
	...
	sysil is used to get the length of the next input record from a file previously input associated with a sysio
	call. the length returned is used to establish a buffer for a subsequent sysin call.  sysil also indicates to the
	...
	note that it is the sysil call (not the sysio call) which causes the file to be opened as required for the first
...
/*
	...
	sysin is used to read a record from the file which was referenced in a prior call to sysil (i.e. these calls
	always occur in pairs). the buffer provided is an scblk for a string of length set from the sysil call.
	if the actual length read is less than this, the length field of the scblk must be modified before returning
	unless buffer is right padded with zeroes.  it is also permissible to take any of the alternative
...
/*
	...
	sysio is called in response to a snobol4 input or output function call except when file arg1 and file arg2
	are both null.  its call always follows immediately after a call
	of sysfc. if sysfc requested allocation of an fcblk, its address will be in wa.
	for input files, non-zero values of _r_ should be copied to wc for use in allocating input buffers. if _r_
	is defaulted or not implemented, wc should be zeroised.  once a file has been opened, subsequent input(),output()
	calls in which the second argument is identical with that in a previous call, merely associate the additional
	variable name (first argument) to the file and do not result in re-opening the file.
	...
	the second error return is used if the file named exists but input/output from the file is not allowed. for
	example, the standard output file may be in this category as regards input association.
*/
	ejc
...
/*
	...
	sysld is called in response to the use of the snobol4 load function. the named function is loaded (whatever
	this means), and a pointer is returned. the pointer will be used on subsequent calls to the function (see sysex).
	...
	the significance of the pointer returned is up to the system interface routine. the only restriction is that
	if the pointer is within dynamic storage, it must be a proper block pointer.
*/
	ejc
...
/*
	...
	sysmm is called in an attempt to allocate more dynamic memory. this memory must be allocated contiguously with
...
/*
	...
	because of the method of garbage collection, no spitbol object is allowed to occupy more bytes of memory than
	the integer giving the lowest address of dynamic (garbage collectable) memory. mxlen is the name used to
	refer to this maximum length of an object and for most users of most implementations, provided dynamic memory
	starts at an address of at least a few thousand words, there is no problem.

	if the default starting address is less than say 10000 or 20000, then a load time option should be provided where a
	user can request that he be able to create larger objects. this routine informs spitbol of this request if
	any. the value returned is either an integer representing the desired value of mxlen (and hence the
	minimum dynamic store address which may result in non-use of some store) or zero if a default is acceptable
	in which mxlen is set to the lowest address allocated to dynamic store before compilation starts.
...
/*
	...
	note that it is the sysou call (not the sysio call) which causes the file to be opened as required for the first
...
/*
	...
	if spitbol is run from an online terminal, osint can request that messages such as copies of compilation
	errors be sent to the terminal (see syspp). if relevant reply was made by syspp then syspi is called to send such
	messages to the interactive channel.  syspi is also used for sending output to the terminal
...
/*
	...
	provides means for interface to take special actions, such as interrupting execution, breakpointing, stepping,
	and expression evaluation.  these last three options are not presently implemented by the code calling syspl.
...
/*
	...
	syspp is called once during compilation to obtain parameters required for correct printed output format
	and to select other options. it may also be called again after sysxi when a load module is resumed. in this
	case the value returned in wa may be less than or equal to that returned in initial call but may not be
	greater.
	...
	2.   no of lines/page. 0 is preferable for a non-paged device (e.g. online terminal) in which case listing
	    page throws are suppressed and page headers resulting from -title,-stitl lines are kept short.

	3.   an initial -nolist option to suppress listing unless the program contains an explicit -list.

	4.   options to suppress listing of compilation and/or execution stats (useful for established programs) -
	    combined with 3. gives possibility of listing file never being opened.
	...
	7.   an option to choose extended or compact listing format. in the former a page eject and in the latter
	    a few line feeds precede the printing of each of-- listing, compilation statistics, execution
	...
	10.  an intermediate (standard) listing option requiring that page ejects occur in source listings. redundant
	    if extended option chosen but partially extends compact option.
...
/*
	...
	the buffer pointed to is the length obtained from the syspp call and is filled out with trailing blanks. the
	value in wa is the actual line length which may be less than the maximum line length possible. there is no space
	control associated with the line, all lines are printed single spaced. note that null lines (wa=0) are possible
	in which case a blank line is to be printed.

	the error exit is used for systems which limit the amount of printed output. if possible, printing should be
	permitted after this condition has been signalled once to allow for dump and other diagnostic information.
	assuming this to be possible, spitbol may make more syspr calls. if the error return occurs another time, execution
...
/*
	...
	sysrd is used to read a record from the standard input file. the buffer provided is an scblk for a string the
	length of which in characters is given in wc, this corresponding to the maximum length of string which
	spitbol is prepared to receive. at compile time it corresponds to xxx in the most recent -inxxx card
	(default 72) and at execution time to the most recent ,r_r_ (record length) in the third arg of an input()
	statement for the standard input file (default 80).  if fewer than (wc) characters are read, the length
	field of the scblk must be adjusted before returning unless the buffer is right padded with zeroes.
	it is also permissible to take the alternative return after such an adjustment has been made.
	spitbol may continue to make calls after an endfile return so this routine should be prepared to make
...
/*
	...
	reads a record from online terminal for spitbol variable, terminal. if online terminal is unavailable then code the
	endfile return only.

	the buffer provided is of length 258 characters. sysri should replace the count in the second word of the scblk
	by the actual character count unless buffer is right padded with zeroes.
...
/*
	...
	sysrw is used to rewind a file i.e. reposition the file at the start before the first record. the file should be
	closed and the next read or write call will open the file at the start.
...
/*
	...
	sysst is called to change the position of a file pointer. this is accomplished in a system dependent
	manner, and thus the 2nd and 3rd arguments are passed unconverted.
...
/*
	...
	systm is used to obtain the amount of execution time used so far since spitbol was given control. the units
	are described as milliseconds in the spitbol output, but the exact meaning is system dependent. where appropriate,
	this value should relate to processor rather than clock timing values.
...
/*
	...
	called by spitbol function trace() with no args to toggle the system trace switch.	this permits tracing of
...
/*
	...
	when sysxi is called, xl contains either a string pointer or zero. in the former case, the string gives the
	character name of a program. the intention is that spitbol execution should be terminated forthwith and
	the named program loaded and executed. this type of chain execution is very system dependent and implementors may
	choose to omit it or find it impossible to provide.  if (xl) is zero,ia contains one of the following integers

	-1, -2, -3, -4

	    create if possible a load module containing only the impure area of memory which needs to be loaded with
	    a compatible pure segment for subsequent executions.  version numbers to check compatibility should be
	    kept in both segments and checked on loading.  to assist with this check, (xr) on entry is a
	    pointer to an scblk containing the spitbol major version number v.v (see sysid).  the file thus
	...
	create if possible a load module from all of memory. it should be possible to load and execute this module directly.
	...
	sysxi should make a note that no i/o channels, including standard files, have files attached so that calls of sysin,
	 sysou, syspr, sysrd should fail unless new associations are made for the load module.

	at least in the case of the standard output file, it is
	recommended that either the user be required to attach a file or that a default file is attached, since the
	problem of error messages generated by the load module is otherwise severe.

	as a last resort, if spitbol attempts to write to the standard output file and gets a reply indicating that
	such ouput is unacceptable it stops by using an entry to sysej with ending code 998.

	as described below, passing of some arguments makes it clear that load module will use a standard output file.

	if use is made of fcblks for i/o association, spitbol builds a chain so that those in use may be found in sysxi and
	sysej. the nodes are 4 words long. third word contains link to next node or 0, fourth word contains fcblk pointer.
*/
	ejc
/*
	...
	loading and running the load module or returning from jcl command level causes execution to resume at the point
	after the error returns which follow the call of sysxi.  the value passed as exit argument is used to indicate
	...
	+4 or -4 indicate that execution is to continue after creation of the save file or load module, although all
	files will be closed by the sysxi action.  this permits the user to checkpoint long-running programs while
...
/*
	...
	equ  * definitions given at the start of this section.

	note that even if conditional assembly is used to omit some feature (e.g. real arithmetic) a full set of cfp_-
	values must be supplied. use dummy values if genuine ones are not needed.
*/
...
/*
	the following definitions require the supply of either a single parameter if real arithmetic is omitted or
...
.if    .cucf
/*
	the following definition for cfp_u supplies a realistic upper bound on the size of the alphabet.
	cfp_u is used to save space in the scane bsw-iff-esw table and to ease translation storage requirements.
*/
...
/*
	...
	the spitbol program is essentially independent of the definitions of these parameters. however, the
	efficiency of the system may be affected. consequently, these parameters may require tuning for a given version
	the values given in comments have been successfully used.

	e_srs is the number of words to reserve at the end of storage for end of run processing. it should be
	set as small as possible without causing memory overflow in critical situations (e.g. memory overflow termination)
	and should thus reserve sufficient space at least for an scblk containing say 30 characters.

*/
e_srs	equ	*			; 30 words
/*
	e_sts is the number of words grabbed in a chunk when storage is allocated in the static region. the minimum
	permitted value is 256/cfp_b. larger values will lead to increased efficiency at the cost of wasting memory.
*/
e_sts	equ	*			; 500 words
/*
	e_cbs is the size of code block allocated initially and the expansion increment if overflow occurs. if this value
	is too small or too large, excessive garbage collections will occur during compilation and memory may be lost
	in the case of a too large value.
*/
e_cbs	equ	*			; 500 words
/*
	e_hnb is the number of bucket headers in the variable hash table. it should always be odd. larger values will
	speed up compilation and indirect references at the expense of additional storage for the hash table itself.
*/
e_hnb	equ	*			; 127 bucket headers
/*
	e_hnw is the maximum number of words of a string name which participate in the string hash algorithm.
	larger values give a better hash at the expense of taking longer to compute the hash. there is some optimal value.
*/
e_hnw	equ	*			; 6 words
/*
	e_fsp.  if the amount of free space left after a garbage collection is small compared to the total amount of space
	in use garbage collector thrashing is likely to occur as this space is used up.  e_fsp is a measure of the
	minimum percentage of dynamic memory left as free space before the system routine sysmm is called to try to
	obtain more memory.
*/
e_fsp	equ	*			; 15 percent
.if    .csed
/*
	e_sed.  if the amount of free space left in the sediment after a garbage collection is a significant fraction of
	the new sediment size, the sediment is marked for collection on the next call to the garbage collector.
*/
...
/*
	...
	the names of these characters are related to their original representation in the ebcdic set corresponding
...
.fi
/*
	if a delimiter other than ch_cm must be used in the third argument of input(),output() then .ciod should
...
/*
	...
	the following sections describe the detailed format of all possible data blocks in static and dynamic memory.

	every block has a name of the form xxblk where xx is a unique two character identifier. the first word of every
	block must contain a pointer to a program location in the interpretor which is immediately preceded by an address
	constant containing the value bl_xx where xx is the block identifier. this provides a uniform mechanism for
	distinguishing between the various block types.

	in some cases, the contents of the first word is constant for a given block type and merely serves as a pointer
	to the identifying address constant. however, in other cases there are several possibilities for the first
	word in which case each of the several program entry points must be preceded by the appropriate constant.

	in each block, some of the fields are relocatable. this means that they may contain a pointer to another block
	in the dynamic area. (to be more precise, if they contain a pointer within the dynamic area, then it is a pointer
	to a block). such fields must be modified by the garbage collector (procedure gbcol) whenever blocks are compacted
	in the dynamic region. the garbage collector (actually procedure gbcpf) requires that all such relocatable
...
/*
	...
	3)   picture of the block format. in these pictures low memory addresses are at the top of the page. fixed
	    length fields are surrounded by i (letter i). fields which are fixed length but whose length is dependent
	    on a configuration parameter are surrounded by * (asterisk). variable length fields are surrounded by / (slash).

	4)   definition of symbolic offsets to fields in block and of the size of the block if fixed length
	    or of the size of the fixed length fields if the block is variable length.
	    note that some routines such as gbcpf assume certain offsets are equal. the definitions
	    given here enforce this.  make changes to them only with due care.
...
/*
	...
	this table provides a unique identification code for each separate block type. the first word of a block in
	the dynamic area always contains the address of a program entry point. the block code is used as the entry point id
	the order of these codes dictates the order of the table used by the datatype function (scnmt in the constant sec)

	block codes for accessible datatypes

	note that real and buffer types are always included, even if they are conditionally excluded elsewhere.  this main-
	tains block type codes across all versions of spitbol, providing consistancy for external functions.  but note
	that the bcblk is out of alphabetic order, placed at the end of the list so as not to change the block type
...
/*
	...
	references to the fields of data blocks are symbolic (i.e. use the symbolic offsets) with the following
	exceptions.

	1)   references to the first word are usually not symbolic since they use the (x) operand format.

	2)   the code which constructs a block is often not symbolic and should be changed if the corresponding
	    block format is modified.

	3)   the plc and psc instructions imply an offset corresponding to the definition of cfp_f.

	4)   there are non-symbolic references (easily changed) in the garbage collector (procedures gbcpf, blkln).

	5)   the fields idval, fargs appear in several blocks and any changes must be made in parallel to all
	    blocks containing the fields. the actual references to these fields are symbolic with the above listed exceptions.

	6)   several spots in the code assume that the definitions of the fields vrval, teval, trnxt are
	    the same (these are sections of code which search out along a trblk chain from a variable).

	7)   references to the fields of an array block in the array reference routine arref are non-symbolic.

	apart from the exceptions listed, references are symbolic as far as possible and modifying the order or number
...
/*
	...
	blocks which represent callable functions have two common fields at the start of the block as follows.
...
/*
	fcode is a pointer to the location in the interpretor program which processes this type of function call.

	fargs is the expected number of arguments. the actual number of arguments is adjusted to this amount by
	deleting extra arguments or supplying trailing nulls for missing ones before transferring though fcode.
	a value of 999 may be used in this field to indicate a variable number of arguments (see svblk field svnar).
...
/*
	...
	certain program accessible objects (those which contain other data values and can be copied) are given a unique
	identification number (see exsid). this id value is an address integer value which is always stored in word two.
*/
...
/*
	...
	note that a zero idval means that the block is only half built and should not be dumped (see dumpr).
*/
	ejc
/*
	array block (arblk)
	...
	an arblk is built with a call to the functions convert (s_cnv) or array (s_arr).
...
/*
	the bounds and dimension fields are signed integer values and each occupy cfp_i words in the arblk.

	the length of an arblk in bytes may not exceed mxlen.  this is required to keep name offsets garbage collectable

	the actual values are arranged in row-wise order and can contain a data pointer or a pointer to a trblk.
*/
.if    .cnbf
.else
/*
...
/*
	a bcblk is an indirect control header for bfblk.  the reason for not storing this data directly in the
	related bfblk is so that the bfblk can maintain the same skeletal structure as an scblk thus
	facilitating transparent string operations (for the most part).  specifically, cfp_f is the same for
	a bfblk as for an scblk.  by convention, whereever a buffer value is employed, the bcblk is pointed to.

	the corresponding bfblk is pointed to by the bcbuf pointer in the bcblk.

	bclen is the current defined size of the character array in the bfblk.  characters following the offset
...
/*
	the characters in the buffer are stored left justified.  the final word of defined characters is always zero
	...
	note that the offset to the characters of the string is given by cfp_f, as with an scblk.  however, the
	offset which is occupied by the length for an scblk is the total char space for bfblks, and routines which
	deal with both must account for this difference.

	the value of bfalc may not exceed mxlen. the value of bclen is always less than or equal to bfalc.
...
/*
	...
	at any one moment there is at most one ccblk into which the compiler is currently storing code (cdwrd).
...
/*
	the reason that the ccblk is a separate block type from the usual cdblk is that the garbage collector must
...
/*
	...
	cdcod is the start of the actual code. first we describe the code generated for an expression. in an expression,
	elements are fetched by name or by value. for example, the binary equal operator fetches its left argument
	by name and its right argument by value. these two cases generate quite different code and are described
...
/*
	...
	assignment	     (to natural variable) value code for right operand pointer to vrsto field of vrblk

			     (to any other variable) name code for left operand value code for right operand =o_ass
...
/*
	...
	note that in this description, =o_xxx refers to the generation of a word containing the address of another
...
/*
	...
	first comes the code for the statement body.  the statement body is an expression to be evaluated
	by value although the value is not actually required.  normal value code is generated for the body of the
	statement except in the case of a pattern match by value, in which case the following is generated.
	...
	following this we generate code for the failure goto if it is direct or if it is complex, simple failure gotos
	having been handled by an appropriate setting of the cdfal field of the cdblk. the generated code is one
	of the following.
	...
	an optimization occurs if the success and failure gotos are identical and either complex or direct. in this case,
	no code is generated for the success goto and control is allowed to fall into the failure goto on success.
*/
	ejc
/*
...
;	cmtyp is set to indicate the type of expression element as shown by the following table of definitions.
...
;      the remaining types indicate expression elements which can only be evaluated by value (not by name).
...
/*
	...
	a character table block is used to hold logical character tables for use with any,notany,span,break,breakx
	patterns. each character table can be used to store cfp_n distinct tables as bit columns. a bit column
	allocated for each argument of more than one character in length to one of the above listed pattern primitives.
...
/*
	ctchs is cfp_a words long and consists of a one word bit string value for each possible character in the
	internal alphabet. each of the cfp_n possible bits in a bitstring is used to form a column of bit indicators.
	a bit is set on if the character is in the table and off if the character is not present.
*/
	ejc
/*
	datatype function block (dfblk)

	a datatype function is used to control the construction of a program defined datatype object. a call to the
	system function data builds a dfblk for the datatype name

	note that these blocks are built in static because pdblk length is got from dflen field.	if dfblk was in dynamic
	store this would cause trouble during pass two of garbage collection.  scblk referred to by dfnam field is also put
	in static so that there are no reloc. fields. this cuts garbage collection task appreciably for pdblks which are
...
/*
	the contents of the dvtyp field is copied into the cmtyp field of the cmblk for the operator if it is used.

	the cmopn field of an operator cmblk points to the dvblk itself, providing the required entry address pointer ptr.

	for normally undefined operators, the dvopn (and cmopn) fields contain a word offset from r_uba of the function
	block pointer for the operator (instead of o_xxx ptr).  for certain special operators, the dvopn field is not
	required at all and is assembled as zero.

	the left precedence is used in comparing an operator to the left of some other operator. it therefore governs the
	precedence of the operator towards its right operand.

	the right precedence is used in comparing an operator to the right of some other operator. it therefore governs
	...
	(higher) than the right precedence for right (left) associative binary operators.

	the left precedence of unary operators is set to an arbitrary high value. the right value is not required and
...
/*
	...
	1)   binary question mark is lowered and made left assoc- iative to reflect its new use for pattern matching.

	2)   alternation and concatenation are made right associative for greater efficiency in pattern
	    construction and matching respectively. this change is transparent to the snobol4 programmer.

	3)   the equal sign has been added as a low precedence operator which is right associative to reflect its
...
/*
	...
	efuse is used to keep track of multiple use when opsyn is employed. the function is automatically unloaded
...
/*
	...
	in this version of spitbol, an expression can be used in any position which would normally expect a name (for
	example on the left side of equals or as the right argument of binary dot). this corresponds to the creation
	of a pseudo-variable which is represented by a pointer to an expression variable block as follows.
...
/*
	the name of an expression variable is represented by a base pointer to the evblk and an offset of evvar. this
	value appears to be trapped by the dummy trbev block.

	note that there is no need to allow for the case of an expression variable which references an seblk since a
...
/*
	...
	an expression block is built for each expression referenced in a program or created by eval or convert
...
/*
	there are two cases for excod depending on whether the expression can be evaluated by name (see description
...
/*
	...
	ffnxt is used to point to the next ffblk of the same name in the case where there are several fields of the same
...
/*
	...
	an icblk is created for every integer referenced or created by a program. note however that certain internal
	integer values are stored as addresses (e.g. the length field in a string constant block)
...
/*
	...
	a kvblk is used to represent a keyword pseudo-variable.  a kvblk is built for each keyword reference (kwnam).
...
/*
	the name of a keyword variable is represented by a base pointer to the kvblk and an offset of kvvar. the
...
/*
	...
	a name block is used wherever a name must be stored as a value following use of the unary dot operator.
...
/*
	the actual field representing the contents of the name is found nmofs bytes past the address in nmbas.

	the name is split into base and offset form to avoid creation of a pointer into the middle of a block which
	could not be handled properly by the garbage collector.

	a name may be built for any variable (see section on representations of variables) this includes the
...
/*
	...
	a p0blk is used to represent pattern nodes which do not require the use of any parameter values.
...
/*
	pthen points to the pattern block for the subsequent node to be matched. this is a pointer to the pattern
...
/*
	...
	parm1 contains a parameter value used in matching the node. for example, in a len pattern, it is the integer
	argument to len. the details of the use of the parameter field are included in the description of the individual
	match routines. parm1 is always an address pointer which is processed by the garbage collector.
*/
	ejc
/*
	pattern block (two parameters)

	a p2blk is used to represent pattern nodes which require two parameter values.
...
/*
	...
	parm2 is a parameter which performs the same sort of function as parm1 (see description of p1blk).

	parm2 is a non-relocatable field and is not processed by the garbage collector. accordingly, it may
...
/*
	...
	a pdblk represents the data item formed by a call to a datatype function as defined by the system function data.
...
/*
	the pddfp pointer may be used to determine the datatype and the names of the fields if required. the dfblk also
	contains the length of the pdblk in bytes (field dfpdl).  pddfp is non-reloc. because dfblk is in static
...
/*
	...
	a pfblk is created for each call to the define function and a pointer to the pfblk placed in the proper vrblk.
...
/*
	the characters of the string are stored left justified.  the final word is padded on the right with zeros.
	(i.e. the character whose internal code is zero).

	the value of sclen may not exceed mxlen. this ensures that character offsets (e.g. the pattern match cursor)
	can be correctly processed by the garbage collector.

	note that the offset to the characters of the string
	is given in bytes by cfp_f and that this value is automatically allowed for in plc, psc.
	note that for a spitbol scblk, the value of cfp_f is given by cfp_b*schar.
*/
	ejc
/*
	simple expression block (seblk)

	an seblk is used to represent an expression of the form *(natural variable). all other expressions are exblks.
...
/*
	...
	an svblk is assembled in the constant section for each variable which satisfies one of the following conditions.
	...
	if vrblks are constructed for any of these variables, then the vrsvp field points to the svblk (see vrblk)
...
/*
	note that the last five bits correspond in order to the fields which are present (see procedure gtnvr).
...
/*
	the svpre bit allows the compiler to preevaluate a call to the associated system function if all the arguments
	are themselves constants. functions in this category must have no side effects and must never cause failure.
	the call may generate an error condition.

	the svffc bit allows the compiler to generate the special fast call after adjusting the number of arguments. only
...
/*
	...
	svknm		     keyword number
	    ...
	    svknm is present only for a standard keyword assoc.  it contains a keyword number as defined by the
	    keyword number table given later on.

	svfnc		     system function pointer

	    svfnc is present only for a system function assoc.  it is a pointer to the actual code for the system
	    function. the generated code for a fast call is a pointer to the svfnc field of the svblk for the
	    function. the vrfnc field of the vrblk points to this same field, in which case, it serves as the
	    fcode field for the function call.

	svnar		     number of function arguments

	    svnar is present only for a system function assoc.  it is the number of arguments required for a call
	    to the system function. the compiler uses this value to adjust the number of arguments in a fast
	    call and in the case of a function called through the vrfnc field of the vrblk, the svnar field
	    serves as the fargs field for o_fnc.

            a special case occurs if this value is set to 999. this is used to indicate that the function has a variable
	    number of arguments and causes o_fnc to pass control without adjusting the argument count. the only
	    predefined functions using this are apply and item.

	svlbl		     system label pointer

	    svlbl is present only for a standard label assoc.  it is a pointer to a system label routine (l_xxx).
	    the vrlbl field of the corresponding vrblk points to the svlbl field of the svblk.

	svval		     system value pointer

	    svval is present only for a standard value.  it is a pointer to the pattern node (ndxxx) which
	    is the standard initial value of the variable.  this value is copied to the vrval field of the vrblk
*/
	ejc
/*
	svblk (continued)
	...
	the following table gives symbolic names for keyword numbers. these values are stored in the svknm field of
	svblks and in the kvnum field of kvblks. see also procedures asign, acess and kwnam.
...
/*
	...
	a table block is used to represent a table value.  it is built by a call to the table or convert functions.
...
/*
	the table block is a hash table which points to chains of table element blocks representing the elements
	in the table which hash into the same bucket.

	tbbuk entries either point to the first teblk on the chain or they point to the tbblk itself to indicate the
	end of the chain.
*/
	ejc
/*
	...
	a table element is used to represent a single entry in a table (see description of tbblk format for hash table)
...
/*
	tenxt points to the next teblk on the hash chain from the tbbuk chain for this hash index. at the end of the chain,
...
/*
	...
	a trap block is used to represent a trace or input or output association in response to a call to the trace
...
/*
	...
	variable input association
	    ...
	    the value field of the variable points to a trblk instead of containing the data value. in the case
	    of a natural variable, the vrget and vrsto fields contain =b_vra and =b_vrv to activate the check.

	    trtyp is set to trtin

	    trnxt points to next trblk or trval has variable val trter is a pointer to svblk if association is
	    for input, terminal, else it is null.

            trtrf points to the trap block which in turn points to an fcblk used for i/o association.
	...
	variable value trace association
	    ...
	    the value field of the variable points to a trblk instead of containing the data value. in the case of a
            natural variable, the vrget and vrsto fields contain =b_vra and =b_vrv to activate the check.
...
/*
	...
	function call trace
	    ...
	    the pfctr field of the corresponding pfblk is set to point to a trblk.
	...
	function return trace
	    ...
	    the pfrtr field of the corresponding pfblk is set to point to a trblk
	...
	label trace
	    ...
	    the vrlbl of the vrblk for the label is changed to point to a trblk and the vrtra field is
...
/*
	...
	keyword trace
	    ...
	    keywords which can be traced possess a unique location which is zero if there is no trace and
	    points to a trblk if there is a trace. the locations are as follows.
	...
	input/output file arg1 trap block
	    ...
	    the value field of the variable points to a trblk instead of containing the data value. in the case of
	    a natural variable, the vrget and vrsto fields contain =b_vra and =b_vrv. this trap block is used
	    to hold a pointer to the fcblk which an implementation may request to hold information
	    about a file.
...
/*
	...
	a vcblk is used to represent an array value which has one dimension whose lower bound is one. all other arrays
	are represented by arblks. a vcblk is created by the system function array (s_arr) when passed an integer arg.
...
/*
	...
	a variable block is built in the static memory area for every variable referenced or created by a program.
	the order of fields is assumed in the model vrblk stnvr.

	note that since these blocks only occur in the static region, it is permissible to point to any word in
	the block and this is used to provide three distinct access points from the generated code as follows.
...
/*
	...
	an xnblk is a block representing an unknown (external) data value. the block contains no pointers to other
	relocatable blocks. an xnblk is used by external function processing or possibly for system i/o routines etc.
	the macro-system itself does not use xnblks.  this type of block may be used as a file control block.
...
/*
	note that the term non-relocatable refers to the contents and not the block itself. an xnblk can be moved around if
...
/*
	...
	an xrblk is a block representing an unknown (external) data value. the data area in this block consists only
	of address values and any addresses pointing into the dynamic memory area must point to the start of other
	data blocks. see also description of xnblk.  this type of block may be used as a file control block.
	see sysfc,sysin,sysou,s_inp,s_oup for details.
...
xrsi_	equ	xrptr			; size of standard fields in xrblk
	ejc
/*
	s_cnv (convert) function switch constants.  the values are tied to the order of the entries in the svctb table
...
/*
	in general, meaningful mnemonics should be used for offsets. however for small integers used often in
...
/*
	the above codes are grouped so that codes for elements which can legitimately immediately precede a unary
...
/*
	...
	see description at start of cmpil procedure for details of use of these locations on the stack.
*/
...
.if    .crel
	ejc
/*
	definition of limits and adjustments that are built by relcr for use by the routines that relocate pointers
	after a save file is reloaded.  see reloc etc. for usage.

	a block of information is built that is used in relocating pointers.  there are rnsi_ instances
	of a rssi_ word structure.  each instance corresponds to one of the regions that a pointer might point into.
...
/*
	...
	all label names are five letters. the order is approximately alphabetical, but in some cases (always
	documented), constants must be placed in some special order which must not be disturbed.

	it must also be remembered that there is a requirement for no forward references which also disturbs the
...
/*
	keyword constant pattern nodes. the following nodes are used as the values of pattern keywords and the initial
	values of the corresponding natural variables. all nodes are in p0blk format and the order is tied to the
...
ndsuc	dac	p_suc			; succeed
	dac	ndnth			;
/*
	null string. all null values point to this string. the svchs field contains a blank to provide for easy default
	processing in trace, stoptr, lpad and rpad.  nullw contains 10 blanks which ensures an all blank word
...
opdvc	dac	o_cnc			; concatenation
	...
	dac	rrcnc			;
/*
	opdvs is used when scanning below the top level to insure that the concatenation will not be later mistaken
...
;	note that the order of the remaining entries is tied to the order of the coding in the scane procedure.
...
.else
/*
	real constants for general use. note that the constants starting at reav1 form a powers of ten table (used in
	gtnum and gtstg)
*/
reav0	drc	+0.0			; 0.0
.if    .cncr
...
scrtn	dac	b_scl			; return
	...
	dtc	/return/
/*
	datatype name table for dtype procedure. the order of these entries is tied to the b_xxx definitions for blocks

	note that slots for buffer and real data types are filled even if these data types are conditionalized out of the
	implementation.	this is done so that the block numbering at bl_ar etc. remains constant in all versions.
*/
...
stndo	dac	o_oun			; ptr to undefined operator err call
	dac	0			; dummy fargs count for call circuit
/*
	standard variable block. this block is used to initialize the first seven fields of a newly constructed vrblk.
...
strtu	dtc	/tu/
/*
	table used by convert function to check datatype name the entries are ordered to correspond to branch table
	in s_cnv
*/
...
/*
	...
	see svblk format for full details of the format. the vrblks are ordered by length and within each length the
...
;	list of svblk pointers for keywords to be dumped. the list is in the order which appears on the dump output.
...
/*
	the working storage section contains areas which are changed during execution of the program. the value
	assembled is the initial value before execution starts.

	all these areas are fixed length areas. variable length data is stored in the static or dynamic regions of the
	allocated data areas.

	the values in this area are described either as work areas or as global values. a work area is used in an
	ephemeral manner and the value is not saved from one entry into a routine to another. a global value is a
	less temporary location whose value is saved from one call to another.

	w_aaa marks the start of the working section whilst w_yyy marks its end.  g_aaa marks the division between
	temporary and global values.

	global values are further subdivided to facilitate processing by the garbage collector. r_aaa through
	r_yyy are global values that may point into dynamic storage and hence must be relocated after each garbage
	collection.  they also serve as root pointers to all allocated data that must be preserved.  pointers between
	a_aaa and r_aaa may point into code, static storage, or mark the limits of dynamic memory.  these pointers
	must be adjusted when the working section is saved to a file and subsequently reloaded at a different address.

	a general part of the approach in this program is not to overlap work areas between procedures even though a
	small amount of space could be saved. such overlap is considered a source of program errors and decreases the
	information left behind after a system crash of any kind.

	the names of these locations are labels with five letter (a-y,_) names. as far as possible the order is kept
	alphabetical by these names but in some cases there are slight departures caused by other order requirements.

	unless otherwise documented, the order of work areas does not affect the execution of the spitbol program.
*/
...
/*
	...
	when a save file is reloaded, these pointers must be adjusted if static or dynamic memory is now
	at a different address.	see routine reloc for additional information.
...
/*
	...
	all the pointers in this section can point to blocks in the dynamic storage area and must be relocated by the
...
;      the remaining pointers in this list are used to point to function blocks for normally undefined operators.
...
/*
	...
	the following section provides services to osint to relocate portions of the workspace.  it is used when
	a saved memory image must be restarted at a different location.
...
/*
	...
	a block of information is built at (xl) that is used in relocating pointers.	there are rnsi_ instances
	of a rssi_ word structure.  each instance corresponds to one of the regions that a pointer might point into.
	the layout of this structure is shown in the definitions section, together with symbolic definitions of the
...
/*
	...
	processes all blocks in the dynamic region.  within a block, pointers to the code section, constant section,
	working globals section, static region, and dynamic region are relocated as needed.
*/
...
/*
	block type switch. note that blocks with no relocatable fields just return to rld05 to continue to next block.

	note that dfblks do not appear in dynamic, only in static.  ccblks and cmblks are not live when a save file is
	created, and can be skipped.

	further note:  static blocks other than vrblks discovered while scanning dynamic must be adjusted at this time.
...
/*
	...
	if the efcod word points to an xnblk, the xnblk type word will not be adjusted.  since this is implementation
...
/*
	...
	this block contains a ptr to a dfblk in the static rgn.  because there are multiple ffblks pointing to the same
	dfblk (one for each field name), we only process the dfblk when we encounter the ffblk for the first field.
	the dfblk in turn contains a pointer to an scblk within static.
*/
...
/*
	...
	in p2blks, parm2 contains either a bit mask or the name offset of a variable.  it never requires relocation.
*/
...
/*
	...
	note that the dfblk pointed to by this pdblk was processed when the ffblk was encountered.  because the
	data function will be called before any records are defined, the ffblk is encountered before any corresponding pdblk.
*/
...
/*
	...
	the list of boundaries and adjustments pointed to by register xl is created by a call to relcr, which should
...
/*
      ...
      only vrblks on the hash chain and any profile block are ;      processed.  other static blocks (dfblks) are processed
      during processing of dynamic blocks.

      global work locations will be processed at this point, so pointers there can be relied upon.
*/
...
;      pointers between a_aaa and r_yyy are examined and ;      adjusted if necessary.  the pointer kvrtn is also
;      adjusted although it lies outside this range.  ;      dname is explicitly adjusted because the limits
;      on dynamic region in stack are to the area actively ;      in use (between dnamb and dnamp), and dname is outside
;      this range.
...
/*
      ...
      the following section receives control from the system at the start of a run with the registers set as follows.
...
;      now convert free store percentage to a suitable factor for easy testing in alloc routine.
...
;      now convert free sediment percentage to a suitable factor for easy testing in gbcol routine.
...
;      now compute starting address for dynamic store and if necessary request more memory.
...
;      here to store values which mark initial division of data area into static and dynamic
...
;      loop here if necessary till enough memory obtained so that dname is above dnamb
...
;      merge when listing file set for execution.  also merge here when restarting a save file or load module.
...
/*
	this section includes all routines which can be accessed directly from the generated code except system functions.

	all routines in this section start with a label of the form o_xxx where xxx is three letters. the generated code
	contains a pointer to the appropriate entry label.

	since the general form of the generated code consists of pointers to blocks whose first word is the address of the
	actual entry point label (o_xxx).

	these routines are in alphabetical order by their entry label names (i.e. by the xxx of the o_xxx name)
...
/*
	...
	this routine is executed at the start of a complex or direct failure goto to trap a subsequent fail (see exfal)
*/
...
/*
	...
	the pattern built by binary dollar is a compound pattern.  see description at start of pattern match section for
...
/*
	...
	the pattern built by binary dot is a compound pattern.  see description at start of pattern match section for
	details of the structure which is constructed.
*/
...
/*
	...
	o_pms is used in place of o_pmv when the pattern match occurs at the outer (statement) level since in this
...
/*
	...
	this entry points is used if the evaluation of an expression, initiated by the evalx procedure, returns
...
/*
	...
	when this routine gets control, the following stack entries have been made (see end of match routine p_nth)
...
/*
	...
	note that if a setexit trap is operating then transfer to system label continue
	will result in looping here.  difficult to avoid except with a considerable overhead which is not worthwhile or
...
/*
	the first word of every block in dynamic storage and the vrget, vrsto and vrtra fields of a vrblk contain a
	pointer to an entry point in the program. all such entry points are in the following section except those for
	pattern blocks which are in the pattern matching segment later on (labels of the form p_xxx), and dope vectors
	(d_xxx) which are in the dope vector section following the pattern routines (dope vectors are used for cmblks).

	the entry points in this section have labels of the form b_xxy where xx is the two character block type for
	the corresponding block and y is any letter.

	in some cases, the pointers serve no other purpose than to identify the block type. in this case the routine
	is never executed and thus no code is assembled.

	for each of these entry points corresponding to a block an entry point identification is assembled (bl_xx).

	the exact entry conditions depend on the manner in which the routine is accessed and are documented with
	the individual routines as required.

	the order of these routines is alphabetical with the following exceptions.

	the routines for seblk and exblk entries occur first so that expressions can be quickly identified from the fact
	that their routines lie before the symbol b_e__.

	these are immediately followed by the routine for a trblk so that the test against the symbol b_t__ checks for
	trapped values or expression values (see procedure evalp)

	the pattern routines lie after this section so that patterns are identified with routines starting at or
	after the initial instruction in these routines (p_aaa).

	the symbol b_aaa defines the first location for block routines and the symbol p_yyy (at the end of the pattern
...
;      the routine for seblk is accessed from the generated code to load the expression value onto the stack.
...
/*
	...
	the cdblk routines are executed from the generated code. there are two cases depending on the form of cdfal.
...
;      the routine for a dfblk is accessed from the o_fnc entry to call a datatype function and build a pdblk.
...
;      the routine for an efblk is passed control form the o_fnc entry to call an external function.
...
bef11	beq	(xr),=b_scl,bef12	; branch if string result
	...
	bri	(xr)			; execute next code word
/*
	here to return a string result that was not in dynamic.  cannot use the simple word copy above because it will not
...
/*
	...
	the routine for an ffblk is executed from the o_fnc entry to call a field function and extract a field value/name.
...
/*
	...
	the routine for a nmblk is executed from the generated code for the case of loading a name onto the stack
	where the name is that of a natural variable which can be preevaluated at compile time.
...
/*
	...
	the routine for a pfblk is executed from the entry o_fnc to call a program defined function.

	(xl)		     pointer to pfblk

	the following stack entries are made before passing control to the program defined function.
...
/*
	...
	the vrblk routines are executed from the generated code.  there are six entries for vrblk covering various cases
*/
b_vr_	ent	bl__i			; mark start of vrblk entry points
/*
	entry for vrget (trapped case). this routine is called from the generated code to load the value of a variable.
	this entry point is used if an access trace or input association is currently active.
...
/*
	...
	entry for vrsto (error case. this routine is called from the executed code for an attempt to modify the value
...
/*
	...
	entry for vrtra (untrapped case). this routine is called from the executed code to transfer to a label.
...
/*
	...
	entry for vrget (untrapped case). this routine is called from the generated code to load the value of a variable.
...
/*
	...
	entry for vrsto (untrapped case). this routine is called from the generated code to store the value of a variable.
...
/*
	...
	vrtra (trapped case). this routine is called from the generated code to transfer to a label when a label
...
/*
	...
	entry for vrsto (trapped case). this routine is called from the generated code to store the value of a variable.
	this entry is used when a value trace or output association is currently active.
...
/*
	the following section consists of the pattern matching routines. all pattern nodes contain a pointer (pcode)
	to one of the routines in this section (p_xxx).

	note that this section follows the b_xxx routines to enable a fast test for the pattern datatype.
*/
p_aaa	ent	bl__i			; entry to mark first pattern

/*
	the entry conditions to the match routine are as follows (see o_pmn, o_pmv, o_pms and procedure match).
...
/*
	...
	alternatives are handled with the special alternative node whose parameter points to the node to be matched
	...
	in the diagram, the node marked + represents an alternative node and the dotted line from a + node
...
/*
	...
	when a failure occurs, the most recent entry on this stack is popped off to restore the cursor and point
	to the node to be matched as an alternative. the entry at the bottom of the stack points to the following
	special nodes depending on the scan mode.

	anchored mode	     the bottom entry points to the special node ndabo which causes an
			     abort. the cursor value stored with this entry is always zero.

	unanchored mode	     the bottom entry points to the special node nduna which moves the
			     anchor point and restarts the match the cursor saved with this entry
			     is the number of characters which lie before the initial anchor point
			     (i.e. the number of anchor moves).  this entry is three words long and
			     also contains the initial pattern.

	entries are made on this history stack by alternative nodes and by some special compound patterns as described
	later on. the following global locations are used during pattern matching.
...
/*
	...
	some patterns have implicit alternatives and their representation in the pattern structure consists of a
	linked set of nodes as indicated by these diagrams.

	as before, the + represents an alternative node and the dotted line from a + node is the parameter pointer
...
/*
	...
	the alternative node at the head of the structure matches null initially but on subsequent failure ensures attempt


	to match the argument.  before the argument is matched p_aba stacks the cursor, pmhbs and a ptr to p_abb.  if
	the argument cant be matched , p_abb removes this special
	stack entry and fails.

	if argument is matched , p_abc restores the outer pmhbs value (saved by p_aba) .	 then if the argument has left
	alternatives on stack it stacks the inner value of pmhbs and a ptr to ndabd. if argument left nothing on the stack
	it optimises by removing items stacked by p_aba.

	finally a check is made that argument matched more than the null
	string (check is intended to prevent useless looping).

	if so the successor is again the alternative node at the head of the structure , ensuring a possible extra attempt
	to match the arg if necessary.  if not , the successor to alternative is taken so as to terminate the loop.  p_abd
	restores inner pmhbs ptr and fails , thus trying to match alternatives left by the arbno argument.
*/
	ejc
/*
	compound pattern structures (continued)
...
/*
	...
	the function of the match routine for ndpab (p_pab) is simply to unstack itself and fail back onto the stack.

	the match routine for p_pac also sets the global pattern flag pmdfl non-zero to indicate that pattern assignments
	may have occured in the pattern match

	if pmdfl is set at the end of the match (see p_nth), the history stack is scanned for matching ndpab-ndpad
	pairs and the corresponding pattern assignments are executed.

	the function of the match routine for ndpad (p_pad) is simply to remove its entry from the stack and fail.
	this includes removing the special node pointer stored in addition to the standard two entries on the stack.
*/
	ejc
/*
	...
	ndfnb (f_fnb) simply is the failure exit for pattern argument failure, and it pops itself and fails onto the
	stack.

	the match routine p_fnc allows for an optimization when the fence pattern leaves no alternatives.  in this case,
	the ndfnb entry is popped, and the match continues.

	ndfnd (p_fnd) is entered when the pattern fails after going through a non-optimized p_fnc, and it pops the
...
/*
	...
	initial entry for a pattern node is to the routine p_exa.  if the evaluated result of the expression is itself a
	pattern, then the following steps are taken to arrange for proper recursive processing.

	1)   a pointer to the current node (the p_exa node) is stored on the history stack with a dummy cursor.

	2)   a special history stack entry is made in which the node pointer points to ndexb, and the cursor value
	    is the saved value of pmhbs on entry to this node.  the match routine for ndexb (p_exb) restores pmhbs
	    from this cursor entry, pops off the p_exa node pointer and fails.

	3)   the resulting history stack pointer is saved in pmhbs to establish a new level of history stack.

	after matching a pattern, the end of match routine gets control (p_nth). this routine proceeds as follows.

	1)   load the current value of pmhbs and recognize the outer level case by the fact that the associated
	    cursor in this case is the pattern match type code which is less than 3. terminate the match in this
	    case and continue execution of the program.

	2)   otherwise make a special history stack entry in which the node pointer points to the special node
	    ndexc and the cursor is the current value of pmhbs.  the match routine for ndexc (p_exc) resets pmhbs to
	    this (inner) value and and then fails.

	3)   using the history stack entry made on starting the expression (accessible with the current value of
	    pmhbs), restore the p_exa node pointer and the old pmhbs setting. take the successor and continue.

	an optimization is possible if the expression pattern makes no entries on the history stack. in this case,
	instead of building the p_exc node in step 2, it is more efficient to simply pop off the p_exb entry and its
...
/*
	...
	the structure and execution of this pattern are similar to those of the recursive expression pattern matching.

	the match routine for ndimb (p_imb) restores the outer level value of pmhbs, unstacks the saved cursor and fails

	the match routine p_imc uses the current value of pmhbs to locate the p_imb entry. this entry is used to make
	the assignment and restore the outer level value of pmhbs. finally, the inner level value of pmhbs and a
	pointer to the special node ndimd are stacked.

	the match routine for ndimd (p_imd) restores the inner level value of pmhbs and fails back into the stack.

	an optimization occurs if the inner pattern makes no entries on the history stack. in this case, p_imc pops
...
/*
	...
	this is the entry which causes an extension of a breakx match when failure occurs. see section on compound
...
/*
	...
	see section on compound patterns for full structure of breakx pattern. the actual character matching uses a
	break node. however, the entry for the expression argument case is separated to get proper error messages.
...
/*
	...
	see compound patterns description for the structure and algorithms for handling expression nodes.
...
/*
	...
	see compound patterns section for the structure and algorithm for matching this node type.
...
/*
	...
	see compound patterns comments at start of this section for details of scheme
...
/*
	...
	see compound patterns description for details of the structure and algorithm for matching this node type.
...
/*
	...
	see compound patterns description for details of the structure and algorithms for matching this node type.
...
/*
	...
	this routine is entered on successful completion.  see description of expression patterns in compound
	pattern section for handling of recursion in matching.

	this pattern also results from an attempt to convert the null string to a pattern via convert()
...
;      now we must perform pattern assignments. this is done by scanning the history stack for matching ndpab-ndpad pairs
...
;      here for ndpab entry, stack initial cursor note that there must be more entries on the stack.

	mov	-(xs),num01(xt)		; stack initial cursor
	chk				; check for stack overflow
	brn	pnth3			; loop back if ok

;      here for ndpad entry. the starting cursor from the matching ndpad entry is now the top stack entry.
...
/*
	optimize pos if it is the first pattern element, unanchored mode, cursor is zero and pos argument
	is not beyond end of string.  force cursor position and number of unanchored moves.

	this optimization is performed invisible provided the argument is either a simple integer or an
	expression that is an untraced variable (that is, it has no side effects that would be lost by short-
	circuiting the normal logic of failing and moving the unanchored starting point.)
...
/*
	...
	see compound patterns description for the structure and algorithms for matching this node type.
...
/*
	...
	optimize rpos if it is the first pattern element, unanchored mode, cursor is zero and rpos argument
	is not beyond end of string.  force cursor position and number of unanchored moves.

	this optimization is performed invisibly provided the argument is either a simple integer or an
	expression that is an untraced variable (that is, it has no side effects that would be lost by short-
	circuiting the normal logic of failing and moving the unanchored starting point).
...
/*
	...
	the following entry point marks the end of the pattern matching routines and also the end of the entry points
...
/*
	the following section contains the routines for labels which have a predefined meaning in snobol4.

	control is passed directly to the label name entry point.  entry names are of the form l_xxx where xxx is the three
...
;      here if error in evaluation of failure goto. cannot continue back to failure goto!
...
/*
	the following section contains coding for functions which are predefined and available at the snobol level.

	these routines receive control directly from the code or indirectly through the o_fnc, o_fns or cfunc routines.
	in both cases the conditions on entry are as follows

	the arguments are on the stack. the number of arguments has been adjusted to correspond to the svblk svnar field.

	in certain functions the direct call is not permitted and in these instances we also have.

	(wa)		     actual number of arguments in call

	control returns by placing the function result value on on the stack and continuing execution with the next
	word from the generated code.

	the names of the entry points of these functions are of the form s_xxx where xxx is the three letter code for
	the system variable name. the functions are in order alphabetically by their entry names.
*/
	ejc
.if    .c370
...
;      apply does not permit the direct (fast) call so that wa contains the actual number of arguments passed.
...
;      arbno builds a compound pattern. see description at start of pattern matching section for structure formed.
...
/*
	the following code is executed twice. the first time (arptr eq 0), it is used to count the number of elements
	and number of dimensions. the second time (arptr gt 0) is used to actually fill in the dim,lbd fields of the arblk.
*/
...
;      loop to scan out names in first argument. variables in the list are flagged by setting vrget of vrblk to zero.
...
;      here to set value of a variable to null protected variables (arb, etc) are exempt
...
/*
	...
	if the first argument is a table, then we go through an intermediate array of addresses that is sorted to
	provide a result ordered by time of entry in the original table.	see c3.762.
*/
...
;      here we must chain an old ffblk ptr to preserve it in the case of multiple field functions with the same name
...
/*
	...
	sysxi returns 0 in wa when a file has been resumed, 1 when this is a continuation of an exit(4) or exit(-4)
	action.
*/
...
/*
	here after exit(4) or exit(-4) -- create save file or load module and continue execution.

	return integer 1 to signal the continuation of the original execution.
*/
...
/*
      ...
      item does not permit the direct (fast) call so that wa contains the actual number of arguments passed.
*/
...
/*
	...
	come here to analyze the datatype pointer in (xr). this code is used both for arguments (wa=1,2) and for the
...
/*
	...
	now set to search for matching unary or binary operator name as appropriate. note that there are =opbun undefined
...
/*
	...
	the actual replace operation uses an scblk whose cfp_a chars contain the translated versions of all the chars.
	the table pointer is remembered from call to call and the table is only built when the arguments change.

	we also perform an optimization gleaned from spitbol 370.  if the second argument is &alphabet, there is no need to
	to build a replace table.  the third argument can be used directly as the replace table.
*/
...
;      loop to swap chars from end to end.  note that in the case of an odd count, the middle char is not touched.
...
/*
	the following section contains utility routines used for various purposes throughout the system. these differ
	from the procedures in the utility procedures section in they are not in procedure form and they do not return
	to their callers. they are accessed with a branch type instruction after setting the registers to appropriate
	parameter values.

	the register values required for each routine are documented at the start of each routine. registers not
	mentioned may contain any values except that xr,xl can only contain proper collectable pointers.

	some of these routines will tolerate garbage pointers in xl,xr on entry. this is always documented and in
	each case, the routine clears these garbage values before exiting after completing its task.
...
/*
	...
	arref continues by executing the next code word with the result name or value placed on top of the stack.
	to deal with the problem of accessing subscripts in the order of stacking, xl is used as a subscript pointer
...
/*
	...
	cfunc is used to call a snobol level function. it is used by the apply function (s_app), the function
	trace routine (trxeq) and the main function call entry (o_fnc, o_fns). in the latter cases, cfunc is used only
...
/*
	...
	exint continues by executing the next code word which it does by falling through to exixr
*/
exint	rtn
	zer	xl			; clear dud value
...
/*
	...
	exixr continues by executing the next code word which it does by falling through to exits.
*/
exixr	rtn
...
/*
	...
	exsid is used to exit after building any of the following blocks (arblk, tbblk, pdblk, vcblk). it sets the idval.
...
/*
	...
	exvnm exits after stacking a value which is a nmblk referencing the name of a given natural variable.
...
/*
	...
	flpop pops the node and cursor on the stack and then drops through into failp to cause pattern failure
...
/*
	...
	failp is used after failing to match a pattern node.  see pattern match routines for details of use.
...
/*
	...
	match continues by executing the pattern match. see pattern match routines (p_xxx) for full details.
*/
match	rtn
...
/*
	...
	retrn continues by executing the code at the return point the stack is cleaned of any garbage left by other
	routines which may have altered flptr since function entry by using flprt, reserved for use only by
...
/*
	here if &profile = 1. start time must be frigged to appear earlier than it actually is, by amount used before
	the call.
*/
...
/*
	...
	permit up to 10 more statements to be obeyed so that setexit trap can regain control.
...
/*
	...
	terminate run and print statistics.  on entry xr points to ending message or is zero if message	printed already.
*/
stopr	rtn
.if    .csax
...
/*
	the following section contains procedures which are used for various purposes throughout the system.

	each procedure is preceded by a description of the calling sequence. usually the arguments are in registers
	but arguments can also occur on the stack and as parameters assembled after the jsr instruction.
	...
	2)   registers whose entry values are not mentioned may contain any value except that xl,xr may only
	    contain proper (collectable) pointer values.  this condition on means that the called routine
	    may if it chooses preserve xl,xr by stacking.

	3)   registers not mentioned on exit contain the same values as they did on entry except that values in
	    xr,xl may have been relocated by the collector.

	4)   registers which are destroyed on exit may contain any value except that values in xl,xr are proper
	    (collectable) pointers.

	5)   the code pointer register points to the current code location on entry and is unchanged on exit.

	in the above description, a collectable pointer is one which either points outside the dynamic region or
	points to the start of a block in the dynamic region.

	in those cases where the calling sequence contains parameters which are used as alternate return points,
	these parameters may be replaced by error codes assembled with the err instruction. this will result
...
/*
	...
	acess loads the value of a variable. trace and input associations are tested for and executed as required.
	...
	failure can occur if an input association causes an end of file condition or if the evaluation of an expression
...
/*
      ...
      this routines allocates a new buffer.  as the bfblk and bcblk come in pairs, both are allocated here,
      and xr points to the bcblk on return.  the bfblk and bcblk are set to the null buffer, and the idval
      is zero on return.
...
/*
	...
	alocs is used to build a frame for a string block into which the actual characters are placed by the caller.
	all strings are created with a call to alocs (the exceptions occur in trimr and s_rpl procedures).
	...
	the resulting scblk has the type word and the length filled in and the last word is cleared to zero characters
...
/*
	...
	note that the coding ensures that the resulting value of state is always less than dnamb. this fact is used
...
/*
	...
	this routine is used by buffer handling routines to append data to an existing bfblk.
	...
	if more characters are specified than can be inserted, then no action is taken and the second return is taken.
*/
...
/*
	...
	arith is used by functions and operators which expect two numeric arguments (operands) which must both be
	integer or both be real. arith fetches two arguments from the stack and performs any necessary conversions.
...
.else
/*
	for real arguments, control returns to the location specified by the third parameter.
...
/*
	...
	asign performs the assignment of a value to a variable with appropriate checks for output associations and
	value trace associations which are executed as required.  asign also handles the special cases of assignment to
	...
	failure occurs if the evaluation of an expression associated with an expression variable fails.
*/
...
/*
	...
	asinp is like asign and has a similar calling sequence and effect. the difference is that the global pattern
...
/*
	...
	blkln is used by the garbage collector and is not permitted to call gbcol directly or indirectly.

	the first word stored in the block (i.e. at xr) may be anything, but the contents of wa must be correct.
*/
...
/*
	...
	cdgex is passed a pointer to an expression tree (see expan) and returns an expression (seblk or exblk).
*/
.if    .cevb
...
/*
	...
	cdgnm is called during the compilation process to generate code by name for an expression. see cdblk
	description for details of code generated. the input to cdgnm is an expression tree as generated by expan.

	cdgnm is a recursive procedure which proceeds by making recursive calls to generate code for operands.
	...
	wc is set to a non-zero (collectable) value if the expression for which code is generated cannot be
...
/*
	...
	cdgvl is called during the compilation process to generate code by value for an expression. see cdblk
	description for details of the code generated. the input to cdgvl is an expression tree as generated by expan.

	cdgvl is a recursive procedure which proceeds by making recursive calls to generate code for operands.
	...
	if wc is non-zero on entry, then preevaluation is not allowed regardless of the nature of the operand.
...
;      merge here for simple constant (icblk,rcblk,scblk) and for variables corresponding to constant keywords.
...
cgv01	mov	-(xs),wb		; save entry wb
	...
	chk				; check for stack overflow
/*
	prepare to generate code for cmblk. wc is set to the value of cswno (zero if -optimise, 1 if -noopt) to
	start with and is reset non-zero for any non-constant code generated. if it is still zero after generating all
...
/*
	0(xs)	is the offset to the previous word which requires filling in with an
		offset to the following o_slc,o_sld

	1(xs)	is the head of a chain of offset pointers indicating those locations
		to be filled with offsets past the end of all the alternatives
*/
...
/*
      ...
      here for keyword by value. this is constant only if the operand is one of the special system variables with
      the svckw bit set to indicate a constant keyword value. note that the only constant operand by name is a variable
*/
...
/*
	...
	cmgen is a subsidiary procedure used to generate value code for a cmblk ptr from the main code generators.
...
/*
	...
	cmpil is used to convert snobol4 source code to internal form (see cdblk format). it is used both for the initial
	compile and at run time by the code and convert functions this procedure has control for the entire duration of
	initial compilation. an error in any procedure called during compilation will lead first to the error section
	and ultimately back here for resumed compilation. the re-entry points after an error are specially labelled -
	...
	cswxx		     control card switch values are changed when relevant control
			     cards are met.
	...
	r_cim		     current (initial) compiler image (zero for initial compile call)

	r_cni		     used to point to following image.  (see readr procedure).
	...
	scnse		     offset (in r_cim) of most recently scanned element. set zero if not
...
/*
	...
	cmpil also uses a fixed number of locations on the main stack as follows. (the definitions of the actual
	offsets are in the definitions section).

	cmstm(xs)	     pointer to expan tree for body of statement (see expan procedure).

	cmsgo(xs)	     pointer to tree representation of success goto (see procedure scngo)
	...
	cmsop(xs)	     offset to word in previous cdblk to be filled in with forward ptr to next cdblk for success goto.
			     zero if no fill in is required.

	cmsoc(xs)	     same as cmsop for current cdblk.

	cmlbl(xs)	     pointer to vrblk for label of current statement. zero if no label
...
;      loop here after comment or control card also special entry after control card error
...
;      for execute time compile, permit embedded control cards and comments (by skipping to next semi-colon)
...
/*
	...
	here with image available to scan. note that if the input string is null, then everything is ok since null is
...
/*
	...
	null statements and statements just containing a constant subject are optimized out by resetting the
...
;      merge here with cdfal value in wa, simple cdblk also special entry after statement error
...
/*
	...
	at this stage, we chop off an appropriate chunk of the current ccblk and convert it into a cdblk. the remainder
...
;      -dump
;      this is used for system debugging . it has the effect of producing a core dump at compilation time
...
;      -trace
;      provided for system debugging.  toggles the system label trace switch at compile time
...
;      sets value of kvcom so that string comparisons may follow collation sequence determined by the interface.
...
/*
	...
	enevs is used by the external interface to evaluate a string expression, typically for an engine wishing to
...
/*
	...
	engts is passed an object and returns a string with any necessary conversions performed.
...
/*
	...
	dffnc is called whenever a new function is assigned to a variable. it deals with external function use counts.
...
/*
	...
	detaches trblks from i/o associated variables, removes entry from iochn chain attached to filearg1 vrblk and may
	remove vrblk access and store traps.  input, output, terminal are handled specially.
...
/*
	first all natural variable blocks (vrblk) whose values are non-null are linked in lexical order using dmvch as
	the chain head and chaining through the vrget fields.  note that this scrambles store if the process is
	interrupted before completion e.g. by exceeding time  or print limits. since the subsequent core dumps and
	failures if execution is resumed are very confusing, the execution time error routine checks for this event and
	attempts an unscramble. similar precautions should be observed if translate time dumping is implemented.
*/
...
/*
	...
	evali is used by pattern primitives len,tab,rtab,pos,rpos when their argument is an expression value.
	...
	on return, the node pointed to has the integer argument in parm1 and the proper successor pointer in pthen.
...
/*
	...
	evalp is used to evaluate an expression (by value) during a pattern match. the effect is like evalx, but pattern
	variables are stacked and restored if necessary.

	evalp also differs from evalx in that if the result is an expression it is reevaluated. this occurs repeatedly.

	to support optimization of pos and rpos, evalp uses wc to signal the caller for the case of a simple vrblk
	that is not an expression and is not trapped.  because this case cannot have any side effects, optimization is
	possible.
...
;      we can give a fast return if the value of the vrblk is not an expression and is not trapped.
...
/*
	...
	evals is used by span, any, notany, break, breakx when they are passed an expression argument.
	...
	on return, the node pointed to has a character table pointer in parm1 and a bit mask in parm2. the proper
	successor is stored in pthen of this node. thus it is ok for merging with the normal (multi-char string) case.
*/
...
/*
      ...
      if an error occurs in the expression code at execution time, control is passed via error section to exfal
      without returning to this routine.  the following entries are made on the stack before giving control to
the expression code
...
/*
      ...
      exbld is used to build an expression block from the code compiled most recently in the current ccblk.
...
/*
	this loop searches for negation and selection code so that the offsets computed whilst code was in code block
	can be transformed to reduced values applicable in an exblk.
*/
...
/*
	...
	selection or negation found reduce the offsets as needed. offsets occur in words
...
/*
	...
	the expression analyzer (expan) procedure is used to scan an expression and convert it into a tree representation.
	see the description of cmblk in the structures section for detailed format of tree blocks.

	the analyzer uses a simple precedence scheme in which operands and operators are placed on a single stack
	and condensations are made when low precedence operators are stacked after a higher precedence operator. a global
	...
	another global variable (in wc) counts the number of items at one grouping level and is incremented for each
	comma encountered. it is stacked with the level indicator

	the scan is controlled by a three state finite machine.  a global variable stored in wa is the current state.
	...
	0    scanning either the main body of a statement or the text of an expression (from eval call). valid
	    terminators are colon, semicolon. the rescan flag is set to return the terminator on the next scane call.
...
;      we rescan the element and create a concatenation operator. this is the case of the blank concatenation operator.
...
/*
	...
	at outer level in a normal goto this is a terminator otherwise it must terminate a function or grouping
*/
...
/*
	...
	at outer level in a direct goto, this is a terminator.  otherwise it must terminate a subscript list.
*/
...
/*
	...
	remove operators (condense) from stack until no more left at this level or top one has lower precedence.
...
;      the operator dv is stored on the stack and the scan ;      continues after setting the scan state to one.
...
/*
      here for special case of binary =. the syntax allows a null right argument for this operator to be left
      out. accordingly we reset to state zero to get proper action on a terminator (supply a null constant).
*/
...
/*
	...
	expap is passed an expression tree to determine if it is a pattern match. the following are recogized as
...
/*
	...
	expdm uses expop to condense all operators at this syntax level. the stack bottom is recognized from the level
...
/*
	...
	expop is used by the expan routine to condense one operator from the top of the syntax stack. an appropriate
	cmblk is built for the operator (unary or binary) and a pointer to this cmblk is stacked.
...
/*
	...
	filnm takes a statement number and examines the file name table pointed to by r_sfn to find the name of the file
	containing the given statement.	table entries are arranged in order of ascending statement number (there
	is only one hash bucket in this table).	elements are added to the table each time there is a change in
	file name, recording the then current statement number.

	to find the file name, the linked list of teblks is scanned for an element containing a subscript (statement
	number) greater than the argument statement number, or the end of chain.  when this condition is met, the
...
/*
	...
	flstg folds a character string containing upper case characcters to one containing lower case characters.
...
/*
	...
	gbcol performs a garbage collection on the dynamic region all blocks which are no longer in use are eliminated
	by moving blocks which are in use down and resetting dnamp, the pointer to the next available location.
...
/*
	...
	1)   all pointers to blocks in the dynamic area must be accessible to the garbage collector. this means
	    that they must occur in one of the following.

	    a)		     main stack, with current top element being indicated by xs
	...
	2)   all pointers must point to the start of blocks with the sole exception of the contents of the code
	    pointer register which points into the r_cod block.

	3)   no location which appears to contain a pointer into the dynamic region may occur unless it is in
	    fact a pointer to the start of the block. however pointers outside this area may occur and will
	    not be changed by the garbage collector.  it is especially important to make sure that xl
	    does not contain a garbage value from some process
	    carried out before the call to the collector.

	gbcol has the capability of moving the final compacted result up in memory (with addresses adjusted accordingly)
	this is used to add space to the static region. the entry value of wb is the number of bytes to move up.
	the caller must guarantee that there is enough room.  furthermore the value in wb if it is non-zero, must be at
...
/*
	...
	the algorithm, which is a modification of the lisp-2 garbage collector devised by r.dewar and k.belcher
	takes three passes as follows.

	1)   all pointers in memory are scanned and blocks in use determined from this scan. note that this procedure
	    is recursive and uses the main stack for linkage.  the marking process is thus similar to that used in
	    a standard lisp collector. however the method of actually marking the blocks is different.

	    the first field of a block normally contains a code entry point pointer. such an entry pointer
	    can be distinguished from the address of any pointer to be processed by the collector. during garbage
	    collection, this word is used to build a back chain of pointers through fields which point to the block.
	    the end of the chain is marked by the occurence of the word which used to be in the first word of
	    the block. this backchain serves both as a mark indicating that the block is in use and as a list of
	    references for the relocation phase.

	2)   storage is scanned sequentially to discover which blocks are currently in use as indicated by the
	    presence of a backchain. two pointers are maintained one scans through looking at each block. the other
	    is incremented only for blocks found to be in use.  in this way, the eventual location of each block can
	    be determined without actually moving any blocks.  as each block which is in use is processed, the back
	    chain is used to reset all pointers which point to this block to contain its new address, i.e. the
	    address it will occupy after the blocks are moved.  the first word of the block, taken from the end of
	    the chain is restored at this point.

	    during pass 2, the collector builds blocks which describe the regions of storage which are to be
	    moved in the third pass. there is one descriptor for each contiguous set of good blocks. the descriptor
	    is built just behind the block to be moved and contains a pointer to the next block and the number
	    of words to be moved.

	3)   in the third and final pass, the move descriptor blocks built in pass two are used to actually move
	    the blocks down to the bottom of the dynamic region.  the collection is then complete and the next
...
.if    .csed
/*
	the garbage collector also recognizes the concept of sediment.  sediment is defined as long-lived objects
	which percipitate to the bottom of dynamic storage.  moving these objects during repeated collections is
	inefficient.  it also contributes to thrashing on systems with virtual memory. in a typical worst-case
	situation, there may be several megabytes of live objects in the sediment, and only a few dead objects in need of
	collection.  without recognising sediment, the standard collector would move those megabytes of objects downward
	to squeeze out the dead objects.  this type of move would result in excessive thrasing for very little memory
	gain.

	scanning of blocks in the sediment cannot be avoided entirely, because these blocks may contain pointers to
	live objects above the sediment.  however, sediment blocks need not be linked to a back chain as described
	in pass one above.  since these blocks will not be moved, pointers to them do not need to be adjusted.  eliminating
...
.if    .cepp
/*
	is needed for blocks in the sediment.  since block type words point to odd-parity entry addresses, merely incre-
	menting the type word serves to mark the block as pro- cessed.	during pass three, the type words are decre-
	mented to restore them to their original value.
*/
.else
/*
	is needed for blocks in the sediment.  all block type words normally lie in the range b_aaa to p_yyy.	blocks
	can be marked by adding an offset (created in gbcmk) to move type words out of this range.  during pass three the
	offset is subtracted to restore them to their original value.
*/
.fi
	ejc
...
;      gbcol (continued)
/*
	the variable dnams contains the number of bytes of memory currently in the sediment.  setting dnams to zero will
	eliminate the sediment and force it to be included in a full garbage collection.

	gbcol returns a suggested new value for dnams (usually dnamp-dnamb) in xr which the caller can store in dnams if it wishes to maintain the sediment.  that is, data remaining after a garbage
	collection is considered to be sediment.

	if one accepts the common lore that most objects are either very short-lived
	or very long-lived, then this naive setting of dnams probably includes some short-lived objects toward the end
	of the sediment.

	knowing when to reset dnams to zero to collect the sedi- ment is not precisely known.  we force it to zero prior
	to producing a dump, when gbcol is invoked by collect() (so that the sediment is invisible to the user), when
	sysmm is unable to obtain additional memory, and when gbcol is called to relocate the dynamic area up in memory
	(to make room for enlarging the static area).  if there are no other reset situations, this leads to the inexo-
	rable growth of the sediment, possible forcing a modest program to begin to use virtual memory that it otherwise
	would not.

	as we scan sediment blocks in pass three, we maintain aggregate counts of the amount of dead and live storage,
	which is used to decide when to reset dnams.  when the ratio of free storage found in the sediment to total
	sediment size exceeds a threshold, the sediment is marked for collection on the next gbcol call.
*/
.fi
	ejc
...
/*
	...
	now we are ready to start pass two. registers are used as follows in pass two.

	(xr)		     scans through all blocks
	(wc)		     pointer to eventual location

	the move description blocks built in this pass have the following format.

	word 1		     pointer to next move block, zero if end of chain of blocks

	word 2		     length of blocks to be moved in bytes. set to the address of the
			     first byte while actually scanning the blocks.

	the first entry on this chain is a special entry consisting of the two words gbcnm and gbcns. after
	building the chain of move descriptors, gbcnm points to the first real move block, and gbcns is the length of
	blocks in use at the start of storage which need not be moved since they are in the correct position.
*/
.if    .csed
...
;      here for a block in use after processing a series of blocks which were not in use, build new move block.
...
.if    .csed
/*
	decide whether to mark sediment for collection next time.  this is done by examining the ratio of previous sediment
...
/*
	...
	this procedure is used by the garbage collector to process fields in pass one. see gbcol for full details.
	...
	note that although this procedure uses a recursive approach, it controls its own stack and is not recursive.
*/
...
;      here we have a ptr to a block in the dynamic area. link this field onto the reference backchain.
...
.if    .csed
/*
	since sediment blocks are not marked by putting them on the back chain, they must be explicitly marked in another
	manner.	if odd parity entry points are present, mark by temporarily converting to even parity.  if odd parity not
	available, the entry point is adjusted by the value in gbcmk.
*/
...
;      block type switch. note that blocks with no relocatable fields just return to gpf02 here to continue to next fld.
...
/*
	the following code is executed twice. on the first pass, signalled by xr=0, the number of non-null elements in
	the table is counted in wb. in the second pass, where xr is a pointer into the arblk, the name and value are
	entered into the current arblk location provided gtawa is non-zero.  if gtawa is zero, the address of the teblk
...
;      loop through buckets in table block next three lines of code rely on tenxt having a value 1 less than tbbuk.
...
;      here to record teblk address in arblk.  this allows a sort routine to sort by ascending address.
...
/*
	now we need the array prototype which is of the form nn,2 this is obtained by building the string for nn02 and
...
/*
	...
	if a spitbol error occurs during compilation or pre- evaluation, control is passed via error section to exfal
...
/*
	check the last character of the string for colon or semicolon.  these characters can legitimately end an
	expression in open code, so expan will not detect them as errors, but they are invalid as terminators for a
...
/*
	...
	gtint is passed an object and returns an integer after performing any necessary conversions.
...
;      gtnum is given an object and returns either an integer ;      or a real, performing any necessary conversions.
...
;      at this point the only possibility is to convert a string to an integer or real as appropriate.
...
;      here for a non-digit character while attempting to convert an integer, check for trailing blanks or real.
...
;      come here if overflow occurs during collection of integer have to restore wb which cvm may have destroyed.
...
/*
	...
	gtnvr locates a variable block (vrblk) given either an appropriate name (nmblk) or a non-null string (scblk).
...
;      here for system variable case to fill in fields where necessary from the fields present in the svblk.
...
/*
	...
	gtpat is passed an object in (xr) and returns a pattern after performing any necessary conversions
...
/*
	...
	gtrea is passed an object and returns a real value performing any necessary conversions.
...
/*
	...
	gtsmi is passed a snobol object and returns an address integer in the range (0 le n le dnamb). such a value can
	only be derived from an integer in the appropriate range.  small integers never appear as snobol values. however,
...
/*
	...
	gtstb is passed an object and returns it unchanged if it is a buffer block, else it returns it as a string with
...
/*
	...
	gtstg is passed an object and returns a string with any necessary conversions performed.
...
;      here with sign flag set and sign forced negative as required by the cvd instruction.
...
;      merge here after converting integer or real into work area. wb is set to nstmx - (number of chars in result).
...
;      at this point we have (1.0 le x lt 10**10) complete scaling with powers of ten table
...
;      the rounding operation may have pushed us up past 1.0 again, so check one more time.
...
/*
	...
	if the exponent is negative or greater than cfp_s, then we convert the number in the form.

	(neg sign) 0 . (cpf_s digits) e (exp sign) (exp digits)

	if the exponent is positive and less than or equal to cfp_s, the number is converted in the form.

	(neg sign) (exponent digits) . (cfp_s-exponent digits)

	in both cases, the formats obtained from the above rules are modified by deleting trailing zeros after the
	decimal point. there are no leading zeros in the exponent and the exponent sign is always present.
*/
...
/*
	...
	gtvar is used to point to an actual variable location for the detach,input,output,trace,stoptr system functions
...
/*
	...
	hashs is used to convert a string to a unique integer value. the resulting hash value is a positive integer
	...
	if there is more than one character in a word, take the first e_hnw words of the characters from
	the string or all the words if fewer than e_hnw.

	compute the exclusive or of all these words treating them as one word bit string values.

	if there is just one character in a word, then mimic the word by word hash by shifting
	...
	the test on e_hnw is done dynamically. this should be done eventually using conditional assembly,
	 but that would require changes to the build process (ds 8 may 2013).
*/
...
/*
	...
	ident compares two values in the sense of the ident differ functions available at the snobol level.
...
;      here if the type words differ. check if string/buffer comparison
...
;	for all other datatypes, must be differ if xr ne xl merge here for differ
...
;	loop to compare characters. note that wc cannot be zero since all null strings point to nulls and give xl=xr.
...
/*
	...
	note that trter (= trtrf) field of standard i/o variables points to corresponding svblk not to a trblk as is the
...
/*
	...
	this routine will replace a section of a buffer with the contents of a given string.  if the length of the
	section to be replaced is different than the length of the given string, and the replacement is not an append,
	...
	the second alternate exit is taken if the insert would overflow the buffer, or if the insert is out past the
...
/*
	merge here with xr pointing to the scblk or bfblk of the object being inserted, and wa containing the
...
/*
	...
	we are shifting the upper segment down to compact the buffer. (the string length is smaller than the
...
/*
	we are shifting the upper segment up to expand the buffer.  (the string length is larger than the
...
;      merge here when padding ok.  now copy in the insert string to the hole.
...
/*
	here if inserting the same buffer into itself.  have to convert the inserted buffer to an intermediate
...
/*
	...
	note that this procedure establishes the pointers prbuf, gtswk, and kvalp.
*/
...
/*
	...
	used by endfile, eject and rewind to find the fcblk (if any) corresponding to their argument.
...
/*
	...
	ioput sets up input/output  associations. it builds such trace and file control blocks as are necessary and
	calls sysfc,sysio to perform checks on the arguments and to open the files.
...
/*
	...
	no additional trap blocks are used for standard input/out files. otherwise an i/o trap block is attached to second
	arg (filearg1) vrblk. see diagram above for details of the structure built.
...
;      add to iochn chain of associated variables unless already present.
...
/*
	...
	ktrex is used to execute a possible keyword trace. it includes the test on trace and tests for trace active.
...
;      listr is used to list a source line during the initial compilation. it is called from scane and scanl.
...
/*
	...
	on return, the table that maps statement numbers to file names has been updated to include this new file name and
	the current statement number.  the entry is made only if the file name had changed from its previous value.
*/
...
/*
	...
	nexts is used to acquire the next source image at compile time. it assumes that a prior call to readr has input
	a line image (see procedure readr). before the current image is finally lost it may be listed here.
...
/*
	...
	these pattern types all generate a similar node type. so the construction code is shared. see functions section
...
ptin3	exi	2			; take out-of-range error exit
	...
	ejc
/*
	patst -- pattern construction for any,notany, break,span and breakx pattern functions.

	these pattern functions build similar types of nodes and the construction code is shared. see functions section
	...
	note that there is a special call to patst in the evals procedure with a slightly different form. see evals
...
;      note that the call from evals cannot pass an expression since evalp always reevaluates expressions.
...
/*
	...
	to concatenate two patterns, all successors in the left pattern which point to the nothen node must be changed to
	point to the right pattern. however, this modification must be performed on a copy of the left argument rather
	than the left argument itself, since the left argument may be pointed to by some other variable value.

	accordingly, it is necessary to copy the left argument.  this is not a trivial process since we must avoid copying
	nodes more than once and the pattern is a graph structure the following algorithm is employed.

	the stack is used to store a list of nodes which have already been copied. the format of the entries on
	this list consists of a two word block. the first word is the old address and the second word is the address
	of the copy. this list is searched by the pcopy routine to avoid making duplicate copies. a trick is
	used to accomplish the concatenation at the same time.  a special entry is made to start with on the stack. this
	entry records that the nothen node has been copied already and the address of its copy is the right pattern.
...
;      the following loop scans entries in the list and makes sure that their successors have been copied.
...
;      now check for special case of alternation node where parm1 points to a node and must be copied like pthen.
...
/*
	...
	pcopy is called from the pconc procedure to copy a single pattern node. the copy is only carried out if the node
...
/*
	...
	prflr is called to print the contents of the profile table in a fairly readable tabular format.
...
/*
	here if space for profile table not yet allocated.  calculate size needed, allocate a static xnblk, and
	initialize it all to zero.  the time taken for this will be attributed to the current
	statement (assignment to keywd profile), but since the timing for this statement is up the pole anyway, this
...
pflu3	mov	xr,(xs)+		; restore callers xr
	...
	exi				; and return
/*
	here if profile is suppressed because a program defined function is about to be entered, and so the current stmt
...
/*
	...
	since memory allocation is undecided on initial call, terminal cannot be associated. the entry with wc non-zero
...
/*
	...
	prtic is called to print the contents of the standard print buffer to the interactive channel. it is only
	called after prtst has set up the string for printing.  it does not clear the buffer.
...
/*
	...
	prtis puts a line from the print buffer onto the interactive channel (if any) and the standard printer.
	it always prints to the standard printer but does not duplicate lines if the standard printer is
...
/*
	...
	prtin prints the integer value which is in the integer accumulator. blocks built in dynamic storage
...
/*
	...
	prtmi is used to print messages together with an integer value starting in column 15 (used by the routines at
...
/*
	...
	prtmm is used to provide memory usage information in both the end-of-compile and end-of-run statistics.
...
/*
	...
	prtnl prints the contents of the print buffer, resets the buffer to all blanks and resets the print pointer.
...
/*
	...
	prtnm is used to print a character representation of the name of a variable (not a value of datatype name)
...
;      here for natural variable name, recognized by the fact that the name base points into the static area.
...
/*
	...
	now we print an identifying name for the object if one can be found. the following code searches for a natural
	variable which contains this object as value. if such a variable is found, its name is printed, else the value
	of the object (as printed by prtvl) is used instead.

	first we point to the parent tbblk if this is the case of a table element. to do this, chase down the trnxt chain.
*/
prn03	bne	(xl),=b_tet,prn04	; jump if we got there (or not te)
	mov	xl,tenxt(xl)		; else move out on chain
	brn	prn03			; and loop back
/*
	now we are ready for the search. to speed things up in the case of calls from dump where the same name base
	will occur repeatedly while dumping an array or table, we remember the last vrblk pointer found in prnmv. so
...
/*
	...
	here for array. first calculate absolute subscript offsets by successive divisions by the dimension values.
	this must be done right to left since the elements are stored row-wise. the subscripts are stacked as integers.
*/
...
;      loop to print subscripts from stack adjusting by adding the appropriate low bound value from the arblk
...
/*
	...
	prtnv is used by the trace and dump routines to print a line of the form
...
/*
	...
	prints a page throw or a few blank lines on the standard listing channel depending on the listing options chosen.
...
;      compact or interactive channel listing. cant print blanks until check made for headers printed and flag set.
...
/*
	...
	if the standard listing option is selected, insist that an eject be done
...
/*
	...
	prtsn is used to initiate a print trace line by printing asterisks and the current statement number. the actual
	format of the output generated is.

	***nnnnn**** iii.....iiii

	nnnnn is the statement number with leading zeros replaced by asterisks (e.g. *******9****)

	iii...iii represents a variable length output consisting of a number of letter i characters equal to fnclevel.
...
/*
	...
	note that the first word of the block (normally b_scl) is not used and need not be set correctly (see prtvn)
...
;      called to print contents of standard print buffer to online terminal. clears buffer down and resets profs.
...
/*
	...
	prtvl places an appropriate character representation of a data value in the print buffer for dump/trace use.
...
/*
      ...
      for pseudo-variable, just print datatype name (name) for all other names, print dot followed by name rep
*/
...
/*
	...
	readr is used to read the next source image. to process continuation cards properly, the compiler must read one
...
.if    .csfn
/*
	here on end of file or new source file name.  if this is a new source file name, the r_sfn table will
	be augmented with a new table entry consisting of the current compiler statement number as subscript, and the
	file name as value.
*/
...
;      outer nesting of execute-time compile of -include resume with any string remaining prior to -include.
...
/*
	...
	the second argument string block is copied to a result block, and pointers returned to allow the caller to
	proceed with the desired operation if two arguments.

	operations like and/or that do not alter the trailing zeros in the last word of the string block can be
	performed a word at a time.  operations such as compl may either be performed a character at a time or will
...
/*
	...
	note that sbstr is called with a dummy string pointer (pointing into a vrblk or svblk) to copy the name of a
...
/*
	...
	on exit, stmcs and stmct contain the counter value to tested in stmgo.
*/
...
/*
	...
	note that if a call by value specifies a non-existent subscript, the default value is returned without building
	a new teblk.
*/
...
/*
	...
	here for integer or real possibility of overflow exist on twos complement
	machine if hash source is most negative integer or is a real having the same bit pattern.
*/
...
/*
	...
	r_cim		     pointer to string block (scblk) for current input image.

	r_cni		     pointer to next input image string pointer (zero if none).

	r_scp		     save pointer (exit xr) from last call in case rescan is set.

	scnbl		     this location is set non-zero on exit if scane scanned past blanks
			     before locating the current element the end of a line counts as blanks.

	scncc		     cncrd sets this non-zero to scan control card names and clears it
			     on return

	scnil		     length of current input image

	scngo		     if set non-zero on entry, f and s are returned as separate syntax
			     types (not letters) (goto processing). scngo is reset on exit.

	scnpt		     offset to current loc in r_cim

	scnrs		     if set non-zero on entry, scane returns the same result as on the
			     last call (rescan). scnrs is reset on exit from any call to scane.
...
/*
	the following jump is used repeatedly for scanning out the characters of a numeric constant or variable name.
...
/*
	...
	here for operators. on entry, wc points to the table of operator dope vectors and wb is the increment to step
	to the next pair (binary/unary) of dope vectors in the list. on reaching scn46, the pointer has been adjusted to
	point to the appropriate pair of dope vectors.  the first three entries are special since they can occur
...
;      all operators come here (equal merges directly), (wc) points to the binary/unary pair of operator dvblks.
...
/*
	...
	scngf is called from cmpil to scan and analyze a goto field including the surrounding brackets or parentheses.
	for a normal goto, the result returned is either a vrblk pointer for a simple label operand, or a pointer to an
	expression tree with a special outer unary operator (o_goc). for a direct goto, the result returned is a
	pointer to an expression tree with the special outer unary operator o_god.
...
/*
	...
	setvr sets the proper values in the vrget and vrsto fields of a vrblk. it is called whenever trblks are
...
/*
	...
	routine to sort an array or table on same basis as in sitbol. a table is converted to an array, leaving two
	dimensional arrays and vectors as cases to be considered.

	whole rows of arrays are permuted according to the ordering of the keys they contain, and the stride referred to,
	is the the length of a row. it is one for a vector.

	the sort used is heapsort, fundamentals of data structure horowitz and sahni, pitman 1977, page 347.
	  it is an order n*log(n) algorithm. in order to make it stable, comparands may not compare equal.

	this is achieved by sorting a copy array (referred to as the sort array) containing at its high address end,
	byte offsets to the rows to be sorted held in the original array (referred to as the key array).

	sortc, the comparison routine, accesses the keys through these offsets and in the case of equality,
	 resolves it by comparing the offsets themselves. the sort permutes the offsets which are then used in a
	final operation to copy the actual items into the new array in sorted order.  references to zeroth item
	are to notional item preceding first actual item.

	reverse sorting for rsort is done by having the less than test for keys effectively be replaced by a
	greater than test.
...
/*
	separate pre-processing for arrays and vectors done.  to simplify later key comparisons, removal of any trblk
...
srt10	jsr	sorth			; sorth(i,n)
	...
	mov	wc,wa			; i = n
/*
	sorting loop. at this point, a(1) is the largest item, since algorithm initialises it as, and then maintains
	it as, root of tree.
*/
...
;      offsets have been permuted into required order by sort. copy array elements over them.
...
;      copying loop for successive items. sorted offsets are held at end of sort array.
...
/*
	...
	compare two sort keys given their offsets. if equal, compare key offsets to give stable sort.
	note that if srtsr is non-zero (request for reverse sort), the quoted returns are inverted.
	for objects of differing datatypes, the entry point identifications are compared.
...
.else
/*
	first examine for string/buffer comparison.  if so, allow lcomp to compare chars in string and buffer
...
;      here if key1 is a buffer, key2 known not to be a buffer. if key2 is a string, then lcomp can proceed.
...
;      here if key1 is a string, key2 known not to be a string. if key2 is a buffer, then lcomp can proceed.

src13	beq	(xr),=b_bct,src09	; j if keys 1/2 are string/buffer
.if    .cnsc
	brn	src11			; prevent convert of key 1 to number
/*
	here if key1 is not a string or buffer.  examine key2.  if it is a string or buffer, then do not
...
/*
	...
	routine used by sortc to obtain item corresponding to a given field name, if this exists, in a programmer
	defined object passed as argument.  if such a match occurs, record is kept of datatype
	name, field name and offset to field in order to short-circuit later searches on same type. note that
...
/*
	...
	this routine constructs a heap from elements of array, a.  in this application, the elements are offsets to keys in
	a key array.
...
/*
	...
	this procedure is shared by trace and stoptr to either initiate or stop a trace respectively.
...
/*
	...
	trimr is passed a pointer to an scblk which must be the last block in dynamic storage. trailing blanks are
	trimmed off and the dynamic storage pointer reset to the end of the (possibly) shortened block.
	...
	the call with wb zero still performs the end zero pad and dnamp readjustment. it is used from acess if kvtrm=0.
*/
...
/*
	...
	trxeq is used to execute a trace when a fourth argument has been supplied. trace has already been decremented.
	...
	the following stack entries are made before passing control to the trace function using the cfunc routine.
	...
	r_cod and the code ptr are set to dummy values which cause control to return to the trxeq procedure on success
...
/*
	...
	xscan scans out one token in a prototype argument in array,clear,data,define,load function calls. xscan
	calls must be preceded by a call to the initialization procedure xscni. the following variables are used.
	...
	in the case where the end of string is encountered, the string includes all the characters to the end of the
	string. no further calls can be made to xscan until xscni is called to initialize a new argument scan
*/
	ejc
;      xscan (continued)
...
/*
	...
	xscni initializes the scan used for prototype arguments in the clear, define, load, data, array functions. see
...
/*
	...
	the error message is printed as part of the compiler output. this printout includes the offending line (if not
	printed already) and an error flag under the appropriate column as indicated by scnse unless scnse is set to zero.

	after printing the message, the generated code is modified to an error call and control is returned to
	the cmpil procedure after resetting the stack pointer.

	if the error occurs after the end line, control returns in a slightly different manner to ensure proper cleanup.
*/
...
/*
	...
	execute time compilation is initiated through gtcod or gtexp which are called by compile, code or eval.
	before causing statement failure through exfal it is helpful to set keyword errtext and for generality
...
;      pop stack until find flptr for most deeply nested prog. defined function call or call of eval / code.
...
/*
	...
	if errlimit keyword is zero, an abort is signalled, see coding for system label abort at l_abo.

	otherwise, errlimit is decremented and an errtype trace generated if required. control returns either via a jump
	to continue (to take the failure exit) or a specified setexit trap is executed and control passes to the trap.
	if 3 or more fatal errors occur an abort is signalled regardless of errlimit and setexit - looping is all too
	probable otherwise.

	fatal errors include stack overflow and exceeding stlimit.
*/
...
;	interrupted partly through a dump whilst store is in a mess so do a tidy up operation. see dumpr for details.
