...
void help(void)
{
    printf("mcc version " MCC_VERSION " - Tiny C Compiler - Copyright (C) 2001-2006 Fabrice Bellard\n"
           "usage: mcc [-v] [-c] [-o outfile] [-Bdir] [-bench] [-Idir] [-Dsym[=val]] [-Usym]\n"
           ...
           "  -Bdir       set mcc internal library path\n"
...
#define MCC_OPTION_NOSEP   0x0002 /* cannot have space before option and arg */
...
static const MCCOption mcc_options[] = {
    ...
    { "l", MCC_OPTION_l, MCC_OPTION_HAS_ARG | MCC_OPTION_NOSEP },
    { "bench", MCC_OPTION_bench, 0 },
    { "bt", MCC_OPTION_bt, MCC_OPTION_HAS_ARG },
#ifdef CONFIG_MCC_BCHECK
    { "b", MCC_OPTION_b, 0 },
#endif
    ...
    { "m", MCC_OPTION_m, MCC_OPTION_HAS_ARG },
...
int parse_args(MCCState *s, int argc, char **argv)
{
    int optind;
    ...
    while (optind < argc) {
        ...
        option_found:
            ...
            case MCC_OPTION_I:
                if (mcc_add_include_path(s, optarg) < 0)
            ...
            case MCC_OPTION_L:
                ...
                break;
            case MCC_OPTION_B:
                /* set mcc utilities path (mainly for mcc development) */
            ...
            case MCC_OPTION_c:
                multiple_files = 1;
                output_type = MCC_OUTPUT_OBJ;
                break;
            case MCC_OPTION_static:
                s->static_link = 1;
                break;
            case MCC_OPTION_shared:
                output_type = MCC_OUTPUT_DLL;
                break;
            ...
            case MCC_OPTION_run:
                {
                    ...
                    multiple_files = 0;
                    output_type = MCC_OUTPUT_MEMORY;
                }
                break;
            case MCC_OPTION_v:
                do {
                    if (0 == s->verbose++)
                        printf("mcc version %s\n", MCC_VERSION);
                } while (*optarg++ == 'v');
                break;
            case MCC_OPTION_f:
                if (mcc_set_flag(s, optarg, 1) < 0 && s->warn_unsupported)
                    goto unsupported_option;
                break;
            case MCC_OPTION_W:
                if (mcc_set_warning(s, optarg, 1) < 0 && 
                    s->warn_unsupported)
                    goto unsupported_option;
                break;
            case MCC_OPTION_w:
            ...
            case MCC_OPTION_Wl:
                ...
                break;
            case MCC_OPTION_E:
                output_type = MCC_OUTPUT_PREPROCESS;
                break;
            default:
...
int main(int argc, char **argv)
{
    ...
    s = mcc_new();
#ifdef _WIN32
    mcc_set_lib_path_w32(s);
#endif
    output_type = MCC_OUTPUT_EXE;
    outfile = NULL;
    multiple_files = 1;
    files = NULL;
    nb_files = 0;
    ...
    /* if outfile provided without other options, we output an
       executable */
    if (outfile && output_type == MCC_OUTPUT_MEMORY)
        output_type = MCC_OUTPUT_EXE;

    /* check -c consistency : only single file handled. XXX: checks file type */
    if (output_type == MCC_OUTPUT_OBJ && !reloc_output) {
    ...
    } else if (output_type != MCC_OUTPUT_MEMORY) {
        if (!outfile) {
            /* compute default outfile name */
            char *ext;
            const char *name = 
                strcmp(files[0], "-") == 0 ? "a" : mcc_basename(files[0]);
            pstrcpy(objfilename, sizeof(objfilename), name);
            ext = mcc_fileextension(objfilename);
#ifdef MCC_TARGET_PE
...
#endif
            if (output_type == MCC_OUTPUT_OBJ && !reloc_output && *ext)
                strcpy(ext, ".o");
            else
    ...
    for(i = 0; i < nb_files && ret == 0; i++) {
        ...
        if (filename[0] == '-' && filename[1]) {
            if (mcc_add_library(s, filename + 2) < 0) {
    ...
    if (do_bench)
        mcc_print_stats(s, getclock_us() - start_time);

    if (s->output_type == MCC_OUTPUT_PREPROCESS) {
        if (outfile)
            fclose(s->outfile);
    } else if (s->output_type == MCC_OUTPUT_MEMORY) {
        ret = mcc_run(s, argc - optind, argv + optind);
    } else
        ret = mcc_output_file(s, outfile) ? 1 : 0;
 the_end:
