...
static int asm_get_local_label_name(MCCState *s1, unsigned int n)
{
    char buf[64];
    TokenSym *ts;
...
static void asm_new_label1(MCCState *s1, int label, int is_local,
...
static void asm_new_label(MCCState *s1, int label, int is_local)
{
    asm_new_label1(s1, label, is_local, cur_text_section->sh_num, ind);
}

static void asm_free_labels(MCCState *st)
{
    Sym *s, *s1;
    Section *sec;
...
static void asm_parse_directive(MCCState *s1)
{
...
/* assemble a file */
static int mcc_assemble_internal(MCCState *s1, int do_preprocess)
{
    int opcode;
...
static int mcc_assemble(MCCState *s1, int do_preprocess)
{
    ...
    ret = mcc_assemble_internal(s1, do_preprocess);
...
static void mcc_assemble_inline(MCCState *s1, char *str, int len)
{
                               ...
                               int is_output)
{
    ...
    if (tok != ':') {
        ...
        for(;;) {
            ...
            if (tok != TOK_STR)
                expect("string constant");
            op->constraint = mcc_malloc(tokc.cstr->size);
...
static void asm_instr(void)
{
    ...
    /* assemble the string with mcc internal assembler */
    mcc_assemble_inline(mcc_state, astr1.data, astr1.size - 1);
...
static void asm_global_instr(void)
{
    ...
    /* assemble the string with mcc internal assembler */
    mcc_assemble_inline(mcc_state, astr.data, astr.size - 1);
