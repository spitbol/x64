...
#define MCC_OUTPUT_DLL_WITH_PLT
#else
...
void *mcc_get_symbol(MCCState *s, const char *name)
{
    int sym_index;
    ElfW(Sym) *sym;
...
void *mcc_get_symbol_err(MCCState *s, const char *name)
{
    void *sym;
    sym = mcc_get_symbol(s, name);
    if (!sym)
    ...
    if (sym_bind != STB_LOCAL) {
        ...
        if (esym->st_shndx != SHN_UNDEF) {
            ...
            if (sh_num == SHN_UNDEF) {
                ...
                /* gr: Happens with 'mcc ... -static mcctest.c' on e.g. Ubuntu 6.01
                   No idea if this is the correct solution ... */
                goto do_patch;
            } else if (s == mcc_state->dynsymtab_section) {
                          ...
                          int type, int symbol)
{
    ...
    if (!sr) {
        ...
        /* if the symtab is allocated, then we consider the relocation
           are also */
        sr = new_section(mcc_state, buf, SHT_RELX, symtab->sh_flags);
...
/* In an ELF file symbol table, the local symbols must appear below
   the global and weak ones. Since MCC cannot sort it while generating
...
static void relocate_syms(MCCState *s1, int do_resolve)
{
...
static unsigned long add_jmp_table(MCCState *s1, unsigned long val)
{
...
static unsigned long add_got_table(MCCState *s1, unsigned long val)
{
...
static void relocate_section(MCCState *s1, Section *s)
{
    Section *sr;
    ...
    for(rel = qrel;
        ...
        case R_X86_64_32S:
            if (s1->output_type == MCC_OUTPUT_DLL) {
                /* XXX: this logic may depend on MCC's codegen
                   now MCC uses R_X86_64_32 even for a 64bit pointer */
                qrel->r_info = ELFW(R_INFO)(0, R_X86_64_RELATIVE);
...
static void relocate_rel(MCCState *s1, Section *sr)
{
    Section *s;
    ElfW_Rel *rel, *rel_end;
...
static void put_got_offset(MCCState *s1, int index, unsigned long val)
{
    int n;
    unsigned long *tab;
    ...
    if (index >= s1->nb_got_offsets) {
        ...
        while (index >= n)
            n *= 2;
        tab = mcc_realloc(s1->got_offsets, n * sizeof(unsigned long));
        if (!tab)
            error("memory full");
        s1->got_offsets = tab;
...
static void build_got(MCCState *s1)
{
    unsigned char *ptr;
...
static void put_got_entry(MCCState *s1,
...
#if defined(MCC_OUTPUT_DLL_WITH_PLT)
            modrm = 0x25;
#else
...
static void build_got_entries(MCCState *s1)
{
    Section *s, *symtab;
    ElfW_Rel *rel, *rel_end;
    ElfW(Sym) *sym;
...
static Section *new_symtab(MCCState *s1,
...
/* add mcc runtime libraries */
static void mcc_add_runtime(MCCState *s1)
{
#if defined(CONFIG_MCC_BCHECK) || !defined(CONFIG_USE_LIBGCC)
    char buf[1024];
#endif
    ...
    if (s1->do_bounds_check) {
        ...
        *ptr = 0;
        ...
        snprintf(buf, sizeof(buf), "%s/%s", s1->mcc_lib_path, "bcheck.o");
        mcc_add_file(s1, buf);
    ...
    if (!s1->nostdlib) {
        ...
        mcc_add_file(s1, CONFIG_SYSROOT "/lib/libgcc_s.so.1");
#else
        snprintf(buf, sizeof(buf), "%s/%s", s1->mcc_lib_path, "libmcc1.a");
        mcc_add_file(s1, buf);
#endif
    }
    /* add crt end if not memory output */
    if (s1->output_type != MCC_OUTPUT_MEMORY && !s1->nostdlib) {
        mcc_add_file(s1, CONFIG_MCC_CRT_PREFIX "/crtn.o");
    }
}
...
int elf_output_file(MCCState *s1, const char *filename)
{
    ElfW(Ehdr) ehdr;
    FILE *f;
    int fd, mode, ret;
    ...
    if (file_type != MCC_OUTPUT_OBJ) {
        ...
        if (!s1->static_link) {
            ...
            if (file_type == MCC_OUTPUT_EXE) {
                ...
                s1->symtab_to_dynsym = mcc_mallocz(sizeof(int) * nb_syms);
                for(sym = (ElfW(Sym) *)symtab_section->data + 1; 
                    sym < sym_end;
                    sym++) {
                    if (ELFW(ST_BIND)(sym->st_info) != STB_LOCAL) {
    ...
    section_order = mcc_malloc(sizeof(int) * shnum);
    section_order[0] = 0;
    sh_order_index = 1;
    ...
    phdr = mcc_mallocz(phnum * sizeof(ElfW(Phdr)));
...
#error unsupported CPU
    ...
    if (file_type != MCC_OUTPUT_OBJ) {
        ...
        if (file_type == MCC_OUTPUT_EXE)
            ehdr.e_entry = (unsigned long)mcc_get_symbol_err(s1, "_start");
        else
    ...
    if (s1->output_format == MCC_OUTPUT_FORMAT_COFF) {
        mcc_output_coff(s1, f);
    } else
#endif
    if (s1->output_format == MCC_OUTPUT_FORMAT_ELF) {
        ...
        mcc_output_binary(s1, f, section_order);
    }
    fclose(f);

    ret = 0;
 the_end:
    mcc_free(s1->symtab_to_dynsym);
    mcc_free(section_order);
    mcc_free(phdr);
    mcc_free(s1->got_offsets);
    return ret;
}

int mcc_output_file(MCCState *s, const char *filename)
{
    int ret;
#ifdef MCC_TARGET_PE
...
static int mcc_load_object_file(MCCState *s1, 
...
static int mcc_load_alacarte(MCCState *s1, int fd, int size)
{
...
/* load a '.a' file */
static int mcc_load_archive(MCCState *s1, int fd)
{
    ArchiveHeader hdr;
    char ar_size[11];
    char ar_name[17];
    ...
    for(;;) {
        ...
        if (!strcmp(ar_name, "/")) {
            ...
            if(s1->alacarte_link)
                return mcc_load_alacarte(s1, fd, size);
...
static int mcc_load_dll(MCCState *s1, int fd, const char *filename, int level)
{ 
    ElfW(Ehdr) ehdr;
    ...
    dllref = mcc_mallocz(sizeof(DLLReference) + strlen(soname));
    dllref->level = level;
...
static int ld_add_file_list(MCCState *s1, int as_needed)
{
    char filename[1024];
    int t, ret;
...
/* interpret a subset of GNU ldscripts to handle the dummy libc.so
   files */
static int mcc_load_ldscript(MCCState *s1)
{
    char cmd[64];
    char filename[1024];
    int t, ret;
