...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len)
{
    ...
    ts = mcc_malloc(sizeof(TokenSym) + len);
    table_ident[i] = ts;
    ts->tok = tok_ident++;
    ts->sym_define = NULL;
    ts->sym_label = NULL;
...
static int mcc_peekc_slow(BufferedFile *bf)
{
    int len;
...
static int *tok_str_realloc(TokenString *s)
{
    ...
    str = mcc_realloc(s->str, len * sizeof(int));
    if (!str)
        error("memory full");
    s->allocated_len = len;
    s->str = str;
...
static CachedInclude *search_cached_include(MCCState *s1,
...
static inline void add_cached_include(MCCState *s1, int type, 
...
#endif
    e = mcc_malloc(sizeof(CachedInclude) + strlen(filename));
    if (!e)
        return;
    e->type = type;
...
static void pragma_parse(MCCState *s1)
{
    int val;
    ...
    case TOK_INCLUDE_NEXT:
        ...
        for (i = -2; i < n; ++i) {
            ...
            if (i == -2) {
                ...
                if (c != '\"')
                    continue;
                size = mcc_basename(file->filename) - file->filename;
...
/* better than nothing, but needs extension to handle '-E' option
   correctly too */
static void preprocess_init(MCCState *s1)
{
...
static int mcc_preprocess(MCCState *s1)
{
    Sym *define_start;
