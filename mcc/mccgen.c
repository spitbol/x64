...
#if defined(MCC_TARGET_ARM) && !defined(MCC_ARM_VFP)
            CValue check;
#endif
...
static void parse_attribute(AttributeDef *ad)
{
    ...
    while (tok != ')') {
        ...
        case TOK_SECTION2:
            ...
            if (tok != TOK_STR)
                expect("section name");
            ad->section = find_section(mcc_state, (char *)tokc.cstr->data);
            next();
            skip(')');
            break;
        case TOK_ALIGNED1:
        ...
        case TOK_UNUSED1:
        case TOK_UNUSED2:
            /* currently, no need to handle it because mcc does not
            ...
            while (tok != '}') {
                ...
                while (1) {
                    ...
                    if (ad.aligned) {
                        ...
                        align = 1;
                    } else if (*mcc_state->pack_stack_ptr) {
                        if (align > *mcc_state->pack_stack_ptr)
                            align = *mcc_state->pack_stack_ptr;
                    }
                    lbit_pos = 0;
                    if (bit_size >= 0) {
                        bt = type1.t & VT_BTYPE;
...
static void unary(void)
{
 ...
 tok_next:
    ...
    case TOK_STR:
        ...
        t = VT_BYTE;
    str_init:
        if (mcc_state->warn_write_strings)
            t |= VT_CONSTANT;
        type.t = t;
        mk_pointer(&type);
        type.t |= VT_ARRAY;
    ...
    case TOK_builtin_frame_address:
        {
            ...
            if (tokc.i != 0) {
                error("MCC only supports __builtin_frame_address(0)");
            }
            next();
            skip(')');
            type.t = VT_VOID;
    ...
    tok_identifier:
        ...
        if (!s) {
            ...
            if (mcc_state->warn_implicit_function_declaration)
                                   ...
                                   int has_init, int v, int scope)
{
    ...
    if ((r & VT_VALMASK) == VT_LOCAL) {
        sec = NULL;
        if (mcc_state->do_bounds_check && (type->t & VT_ARRAY))
            loc--;
        loc = (loc - size) & -align;
        addr = loc;
        /* handles bounds */
